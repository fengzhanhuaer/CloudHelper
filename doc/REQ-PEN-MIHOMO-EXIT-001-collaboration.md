...
- REQ-PEN-MIHOMO-EXIT-001-R06: 主控 `/mng/route` 新增独立“二次分流”Tab，仅管理已创建特殊出口的Clash/Mihomo多订阅源、默认动作、分流规则、聚合预览和同步状态；页面不得自动选择探针，必须先选择特殊探针，再按单列展示基础配置、Clash订阅源、二次分流规则、该探针的聚合路由规则和运行状态；该Tab不得创建探针或生成安装信息。
...
- AC-05: 二次分流Tab可完成已创建特殊出口的多订阅源、规则、默认动作和聚合预览；每个订阅源可独立命名、启用、删除并按需配置请求头，公开订阅无需请求头；订阅秘密不明文回显；未选择探针时详情隐藏，选择后页面按基础配置、Clash订阅源、二次分流规则、聚合路由规则、运行状态单列展示，聚合与状态只属于当前探针；状态同时核对desired/applied revision与hash、BuildKind、健康和exit_ready，并显示探针/Mihomo版本、最后订阅刷新和最后应用错误；页面不存在探针创建或安装入口。
...
| MOD-PEN-03 | 主控管理API/UI | 探针页统一创建/安装；路由页配置、订阅刷新、聚合预览和状态 | 管理会话 | JSON与页面 |
...
- 输入: IF-PEN-001至003。
- 输出: 可编辑配置、聚合预览、desired/applied状态、exit_ready、版本和错误。
...
#### 1.4.6 结论
- TASK-PEN-010已完成：二次分流页先选择特殊探针，再显示配置及该探针的聚合和运行状态；聚合与状态移至配置之后并限定为当前节点；Code证据已回写并再次通过最终门禁。
...
| R06 | 主控二次分流Tab | 1.2 | UNIT-PEN-04,05 | TASK-PEN-002,008,009,010 | 已完成 | 路由页仅保留配置和状态，无创建/安装入口；先选择探针，再按配置、订阅、规则、聚合、状态单列展示 |
...
| Code章节存在 | 通过 | 第2章 | TASK-PEN-000至010证据完整 |
...
| 需求任务覆盖完整 | 通过 | 1.5矩阵 | 无 |
| 任务自测覆盖完整 | 通过 | TEST-PEN-000至010 | 每项任务均有测试或未执行原因 |
| 修改文件在允许范围内 | 通过 | 2.5.4与补充后的1.4.1逐项核对 | 状态/命令/许可证必要落点已显式列出 |
| 测试失败已记录缺陷 | 通过 | DEFECT-PEN-001至014；2.5.6并行抖动记录 | 最终整改和稳定复跑全部通过 |
...
- 责任方: 无。
- 关闭要求: 无。
- 整改关闭: TASK-PEN-010已完成；主控全量、vet、页面脚本及桌面/移动浏览器探针选择与派生状态交互均通过。
...
#### 2.5.3 执行报告
- TASK-PEN-000至010全部实现；同一`probe_node`源码形成`normal`和`mihomo_exit`两个产品，特殊版仅Linux amd64且不启动普通本地控制台、代理、系统DNS、同步、DDNS或平台TUN。
...
|---|---|---|---|---|---|---|---|
| 无 | TASK-PEN-000至010 | 无 | 无未处理任务包缺口或接口冲突 | 无 | 进入最终门禁 | 已完成 | 无需整改 |
