...
- 当前阶段: Code已完成全局Clash订阅库、主控/指定探针可选拉取、安全订阅重定向、全部磁贴管理页快捷切换、定向同步、即时回报和特殊出口连通性磁贴，Architect增量门禁已通过
- 最近更新角色: Code
- 最近更新时间: 2026-08-15T21:08:16+08:00
...
- REQ-PEN-MIHOMO-EXIT-001-R06: 主控 `/mng/route` 的“二次分流”Tab包含“Clash订阅”和“探针出口”两个子Tab。Clash订阅是主控级全局资源，不属于任何特殊探针，供所有特殊探针选择；探针出口子Tab不得自动选择探针，选择后按单列展示已指向该探针的路由规则出口选择和运行状态。路由规则出口项只展示规则名称和出口选择，不展示匹配条目，也不提供添加、删除或编辑规则入口，不展示聚合规则；该Tab不得创建探针或生成安装信息。
...
- REQ-PEN-MIHOMO-EXIT-001-R10: 主控独占保存一份最多32个Clash配置源的全局订阅库，自动协商Clash/Mihomo YAML格式，使用结构化YAML解析、限制下载并防止SSRF；每个配置源独立刷新并原子替换自身节点，失败或与其他来源节点重名时保留该源last-known-good。特殊出口只接收其规则实际选择的规范化代理节点和从原路由规则编译的匹配条目，不接收配置URL、订阅归属或其他探针未使用的节点。
...
- REQ-PEN-MIHOMO-EXIT-001-R13: 管理端和管理API使用同一简化模型；Clash配置解析后的完整节点及认证秘密只保存在主控全局私有节点库中，管理响应仅返回可选节点名称及订阅来源。探针出口写入只接受`route_rule_id/target`，其中target为DIRECT或全局节点库中的具体节点；后端必须确认该原路由规则仍以当前特殊探针为出口，并只把该探针实际引用的节点秘密编入私有快照。
...
- REQ-PEN-MIHOMO-EXIT-001-R16: Clash配置源不提供用户手工请求头配置；所选拉取探针请求订阅时自动发送固定的Clash Meta/Mihomo格式协商标识，优先取得标准YAML。管理API不得接受`headers`或`clear_headers`，管理页面不展示请求设置或请求头状态；旧持久化请求头不再参与抓取或订阅源指纹，并在后续规范化保存时自然移除。
...
- REQ-PEN-MIHOMO-EXIT-001-R18: 保存或刷新特殊出口配置时，主控必须定向向当前特殊探针发送`route_config_sync`并在响应中返回准确的dispatched/offline/failed结果；探针成功应用后必须立即上报新的applied revision/hash，不得依赖最长60秒的周期上报。页面应等待并刷新同步结果，不能把刚保存后的旧上报直接显示为最终“配置未同步”。
- REQ-PEN-MIHOMO-EXIT-001-R19: Clash订阅单独位于“Clash订阅”子Tab，订阅库不依赖当前探针选择并供所有特殊探针共用。每条Clash配置各自提供“提取节点”按钮，只原子刷新该订阅的节点并保留其他订阅结果；顶部不提供全局提取按钮。每个提取节点必须在主控保留其订阅源ID，节点按订阅分组，探针规则出口下拉框显示“订阅名称 / 节点名称”，保存值仍为原节点名且来源元数据不得进入Mihomo私有快照。特殊探针应通过本机受认证Mihomo API对被路由规则实际选中的非DIRECT节点去重探测，向主控上报连通性、延迟、错误和时间；选中探针的运行状态增加“出口连通性”磁贴展示这些结果。
- REQ-PEN-MIHOMO-EXIT-001-R20: 每个全局Clash订阅可选一个“拉取探针”；不选择时表示用户显式要求主控直接拉取，选择后主控必须通过现有认证控制连接向该桌面探针下发一次性`subscription_fetch`命令，不得在所选探针失败时静默回退主控。主控和探针拉取均须执行HTTPS、合法端口、公开IP固定解析、禁代理、TLS、超时和8 MiB限制，并支持最多5次逐跳重新校验的HTTPS重定向；HTTP 403时只允许自动尝试内置Clash/Mihomo兼容请求标识，不提供手工请求头。探针本地VRoute TUN运行时，固定IPv4拨号前须先建立物理出口旁路，旁路失败时关闭该候选而非经TUN抓取；多IP应按IPv4优先逐个尝试。错误须区分超时、拒绝、不可达、TLS和HTTP状态，且错误及日志不得包含订阅URL。探针结果须校验请求ID、节点身份、回传大小和SHA-256，再按来源指纹、内容格式、跨来源重名、last-known-good和定向同步规则原子提交。订阅URL仅保存在主控，并仅在选择探针时的一次性命令中发送，不进入特殊出口私有快照或探针磁盘。
- REQ-PEN-MIHOMO-EXIT-001-R21: `/mng/settings`、`/mng/probe`、`/mng/backup`、`/mng/notepad`、`/mng/controller-logs`、`/mng/route`、`/mng/cloudflare`和`/mng/tg`页头必须提供磁贴面板全部可进入页面的快捷入口，名称和路由与`/mng/panel`保持一致；每页仅当前入口须有可访问的选中态。快捷入口在宽屏填充页头中部，在390px移动端改为两列且不得遮挡原有标题、页面操作、“返回磁贴面板”和“退出登录”。

#### 1.1.2 需求范围
- 主控特殊出口配置存储、规范化、校验、敏感字段作用域和配置同步。
- 原路由规则到特殊出口二级策略的关联、重编译和失效清理。
- `/mng/route` 二次分流Tab中的只读规则投影和DIRECT/Clash节点选择。
- 全部磁贴管理页共享的页头快捷入口及桌面/移动响应式布局。
- 特殊出口保存定向下发、立即应用回报、订阅来源标识和已选节点连通性/延迟磁贴。
...
- AC-05: 二次分流Tab可在不选择探针时进入独立“Clash订阅”子Tab，完成全局多配置源维护、逐源节点提取和按订阅分组查看；每个配置源可独立命名、启用和删除，只需填写HTTPS URL，格式协商由主控自动完成。“探针出口”子Tab选择探针后仅显示路由规则出口、运行状态和出口连通性，不显示订阅编辑、域名编辑、匹配条目、添加/删除域名组或聚合规则。每条已分配规则只显示名称，并提供DIRECT和全局具体Clash节点选择。
...
- AC-16: 订阅HTTP请求首选`User-Agent: clash.meta`并固定包含接受YAML的`Accept`；仅在HTTP 403时按确定顺序尝试内置Mihomo/Clash Meta兼容标识。管理页面不存在请求设置、请求头JSON或清除请求头控件，保存请求仅提交订阅`id/name/enabled/url/fetch_node_id`。管理API收到旧`headers`或`clear_headers`字段时明确拒绝；标准YAML、Base64兜底、SSRF、物理出口旁路、403回退和原子刷新回归通过。
...
- AC-18: 特殊出口保存接口只向当前node_id下发同步；在线时响应`dispatched=1`，离线时`offline=1`。探针应用成功后触发一次即时状态报告，页面轮询到desired/applied revision与SHA一致后显示就绪；失败或超时保留可诊断状态和last_apply_error。
- AC-19: 两个订阅各含不同节点时，全局订阅库每行提取按钮只替换对应订阅的节点，另一订阅节点及所有探针选择保持不变；失败保持该订阅last-known-good。两个特殊探针均能从同一管理响应选择任一全局节点，页面按订阅分组展示节点，规则选择和连通性磁贴均显示正确来源名称；探针写入载荷仍严格只有`route_rule_id/target`，私有快照只含当前探针实际选中的代理对象。特殊探针只探测当前规则实际选中的非DIRECT节点，Mihomo delay API成功/失败均形成脱敏报告；桌面和390px页面无溢出、重叠或控制台错误。
...
| UNIT-PEN-18 | SpecialExitTargetSync | MOD-PEN-03,06 | 定向同步特殊出口快照并立即回报应用状态 | node_id/控制命令 | 下发和应用状态 |
| UNIT-PEN-19 | SpecialExitConnectivity | MOD-PEN-01,03,08 | 维护全局节点来源并探测实际二级出口 | 订阅节点/规则目标 | 来源和延迟状态 |
| UNIT-PEN-20 | ProbeSubscriptionFetcher | MOD-PEN-03,06 | 通过指定探针安全拉取订阅并校验回传 | fetch_node_id/URL/限制 | 已校验订阅字节或脱敏错误 |
...
##### UNIT-PEN-18
- 单元名称: SpecialExitTargetSync
- 职责: 保存或刷新后定向通知当前特殊探针，并在成功应用后立即上报状态。
- 输入: node_id、controller base URL、route_config_sync控制帧。
- 输出: dispatched/offline/failed下发结果以及最新applied revision/hash报告。
- 处理规则: 特殊出口CRUD只通知目标节点；探针配置应用完成后调用现有线程安全即时报告器；页面按desired revision/hash短轮询状态。
- 异常规则: 离线、发送失败、拉取失败或应用失败均不得伪装为同步成功。

##### UNIT-PEN-19
- 单元名称: SpecialExitConnectivity
- 职责: 保存主控全局订阅库和节点来源，并由特殊探针测量实际已选二级节点的连通性和延迟。
- 输入: subscription_id对应的单订阅解析结果、v3规则目标、Mihomo `/proxies/{name}/delay`。
- 输出: 主控私有节点来源映射和`target/reachable/latency_ms/error/checked_at`运行报告。
- 处理规则: 全局订阅库独立于探针配置；每次只原子替换一个订阅的节点并保留其他来源节点；每个探针快照只裁剪其规则实际选中的代理对象，来源映射不参与快照SHA且不下发；只对去重后的非DIRECT已选目标并发限流探测；节点区按订阅分组，规则下拉和磁贴用订阅名称/节点名称显示结果。
- 异常规则: 单节点失败不影响Mihomo进程健康和其他节点结果；错误脱敏并限制长度。
...
##### UNIT-PEN-20
- 单元名称: ProbeSubscriptionFetcher
- 职责: 主控把订阅的一次性下载任务发给该来源选定的在线桌面探针，由探针使用本机网络出口取回内容。
- 输入: 已认证探针会话、`request_id/fetch_node_id/url/max_bytes/timeout_sec`。
- 输出: 绑定节点身份的Base64内容、原始大小、SHA-256或不含URL的脱敏错误。
- 处理规则: 启用且已配置URL的来源必须选择已登记非Android探针；仅允许HTTPS和显式1至65535端口；探针解析并固定公开IP、保留TLS SNI、禁系统代理和重定向，自动发送Clash Meta UA/YAML Accept，限制20秒下载、8 MiB内容和30秒控制通道回传；主控校验请求ID、认证会话节点、大小与SHA-256后才交给既有解析和原子提交逻辑。
- 异常规则: 探针不存在、Android、离线、旧版不响应、超时、SSRF、HTTP、重定向、过大内容、身份/大小/哈希错配均明确失败；不回退到主控下载，不在探针磁盘或日志保存订阅URL和内容。
...
| TASK-PEN-015 | R06,R10,R16 | UNIT-PEN-01,04,05,06 | `probe_controller/internal/core/probe_special_exit.go`、`probe_special_exit_mng.go`、`mng_pages/route.html`、`probe_special_exit_test.go`、README、安装文档和本协作文档 | 修改/删除 | 删除订阅请求头DTO、持久化解释、管理响应和页面控件；下载端自动设置`User-Agent: clash.meta`及YAML Accept；旧请求头API字段拒绝；专项、全量、vet和桌面/移动Playwright通过 |
...
| TASK-PEN-020 | R10 | UNIT-PEN-20 | `probe_node/subscription_fetch.go`、`subscription_fetch_test.go`及本协作文档 | 修改 | 探针订阅下载器支持HTTPS URL显式合法端口；下载固定到校验后的公开IP并保留TLS SNI、禁代理；默认端口保持443。原“禁重定向”边界由TASK-PEN-023替换为最多5次逐跳重新校验的HTTPS重定向 |
| TASK-PEN-021 | R20 | UNIT-PEN-04,05,20 | 控制端订阅模型/命令等待器/WS结果、`mng_pages/route.html`、探针下载器/控制消息、双端测试及本协作文档 | 修改 | 每源选择拉取探针；主控下发一次性抓取命令并校验结果；探针安全下载后回传，不回退主控抓取 |
| TASK-PEN-022 | R21 | UNIT-PEN-05 | `mng_pages/route.html`、页面契约测试及本协作文档 | 修改 | 页头增加九个磁贴快捷入口并高亮当前页；桌面填充留白、移动两列；真实点击跳转和浏览器布局验证通过 |
| TASK-PEN-023 | R20,R21 | UNIT-PEN-04,05,20 | 主控/探针订阅下载器、special-exit校验与调度、共享管理页渲染器、八个磁贴管理页模板/handler、双端测试及本协作文档 | 修改 | 空拉取探针保存并由主控安全下载；选中探针仍严格定向且支持安全重定向；下拉显示主控默认和探针在线状态；八个管理页共享九个入口且各自唯一高亮；双端全量和桌面/390px浏览器流程通过 |
| TASK-PEN-024 | R16,R20 | UNIT-PEN-20 | 主控/探针订阅下载器、双端专项测试及本协作文档 | 修改 | 403仅自动回退内置Clash/Mihomo请求标识；探针TUN运行时固定IPv4先建物理旁路且失败关闭；多IP逐个拨号；网络错误分级且URL脱敏；主控全量/vet及普通/特殊探针全量通过 |
...
| R06 | 主控二次分流Tab | 1.2 | UNIT-PEN-04,05 | TASK-PEN-002,008,009,010,012,016,019 | 已完成 | 全局Clash订阅与探针出口为独立子Tab；探针出口选择后只显示规则出口和运行状态 |
...
| R10 | 安全订阅与秘密 | 1.2 | UNIT-PEN-01,04,06,12,20 | TASK-PEN-001,002,005,009,013,015,020,021 | 已完成 | 多订阅、自动格式协商、原子合并、探针侧HTTPS/SSRF/脱敏/作用域/0600运行秘密均通过 |
...
| R18 | 保存定向下发和立即回报 | 1.2 | UNIT-PEN-18 | TASK-PEN-018 | 已完成 | 不再等待周期报告消除短暂假未同步 |
| R19 | 全局订阅来源与出口连通性 | 1.2 | UNIT-PEN-05,19 | TASK-PEN-019 | 已完成 | 来源仅主控可见，延迟由特殊探针经Mihomo实测 |
| R20 | 主控/指定探针可选拉取订阅 | 1.2 | UNIT-PEN-04,05,20 | TASK-PEN-021,023 | 已完成 | 空选主控安全拉取；选中探针经认证控制连接回传；两条路径逐跳校验HTTPS重定向并原子解析 |
| R21 | 全部磁贴管理页快捷入口 | 1.2 | UNIT-PEN-05 | TASK-PEN-022,023 | 已完成 | 八个管理页共享九个入口，当前页唯一高亮，桌面/移动布局通过 |
...
| IF-PEN-002 | R06,R10,R13-R15,R20 | Subscription Refresh | 路由页 | 主控 | subscription_id | 该来源刷新后的节点名称池/revision/hash及跳过数 | 已完成 | 先由来源指定探针安全拉取，再逐源原子提交；识别标准YAML和明文/Base64 AnyTLS，节点级过滤AnyTLS+Reality，URL及节点秘密不进入特殊出口快照 |
...
| IF-PEN-011 | R18 | Targeted Route Config Sync | 路由页/主控 | 当前特殊探针 | node_id/controller base URL | 下发统计及即时applied状态 | 已完成 | 探针配置保存不广播无关节点；全局订阅刷新只通知实际受影响节点 |
| IF-PEN-012 | R19 | Global Clash Library and Special Exit Connectivity | 路由页 | 主控全局库/特殊探针/Mihomo | subscription_id、已选非DIRECT节点 | 来源标签、可达性、延迟、错误和时间 | 已完成 | 使用受认证回环Mihomo delay API |
| IF-PEN-013 | R20 | Probe Subscription Fetch | 路由页/主控 | 指定桌面探针 | request_id、URL、超时和大小限制 | Base64内容、原始大小、SHA-256或脱敏错误 | 已完成 | 只走认证控制连接；探针不落盘URL/内容；离线、超时或旧版不支持时明确失败 |
...
| 主控/指定探针可选拉取订阅 | 通过 | TASK-PEN-021、023、TEST-PEN-021、023、DEFECT-PEN-025、026 | 空选主控拉取、指定探针认证回传、双方安全重定向、节点在线状态和原子应用证据完整 |
| 全部磁贴管理页快捷入口 | 通过 | TASK-PEN-022、023、TEST-PEN-022、023 | 八页共享九个链接、各页唯一选中态、真实跳转及1440/390响应式证据完整 |
...
- 责任方: Code、Architect。
- 关闭要求: 已满足。
- 整改关闭: TASK-PEN-016至019与DEFECT-PEN-020至023已完成。
...
| R06,R10,R16 | TASK-PEN-015 | `probe_special_exit.go`、`probe_special_exit_mng.go`、`mng_pages/route.html`、`probe_special_exit_test.go`、README、安装文档和本协作文档 | 已完成 | 已完成 | TEST-PEN-015 | 下载端固定发送Clash Meta UA及YAML Accept；DTO、持久化解释、管理响应和页面均无手工请求头；旧API字段拒绝 |
...
| R18 | TASK-PEN-018 | 主控定向同步、探针即时报告、页面等待同步及测试 | 已完成 | 已完成 | TEST-PEN-018、DEFECT-PEN-022 | 目标探针下发和即时状态闭环通过 |
| R19 | TASK-PEN-019 | 全局订阅库、来源、Mihomo延迟探测、运行报告/磁贴及测试 | 已完成 | 已完成 | TEST-PEN-019、DEFECT-PEN-023 | 全局共享、逐源提取、快照裁剪和磁贴闭环通过 |
| R20 | TASK-PEN-021,023 | 订阅拉取探针配置、主控默认下载、命令/结果通道、双端安全下载/重定向、路由页选择及测试 | 已完成 | 已完成 | TEST-PEN-021,023、DEFECT-PEN-025,026 | 空选主控；选中探针失败时不静默回退 |
| R21 | TASK-PEN-022,023 | 八个磁贴管理页共享页头快捷入口、当前页状态、响应式布局及测试 | 已完成 | 已完成 | TEST-PEN-022,023 | 不修改磁贴面板路由或业务内容 |
...
| IF-PEN-011至013 | R18-R20 | 目标同步、全局订阅/连通性、`probe_subscription_fetch.go`/WS与探针`subscription_fetch.go` | 路由页/主控 | 目标探针/Mihomo | 已完成 | TEST-PEN-018至021 | 每源拉取探针通过认证控制会话请求并回传校验内容；不走主控下载 |
...
| TEST-PEN-020 | R10 | TASK-PEN-020 | 非443 HTTPS订阅端口 | 探针URL校验和固定IP拨号单测、普通/特殊探针全量 | 已完成 | `https://subscription.example:8443`通过校验并拨号`1.1.1.1:8443`；端口0和65536拒绝；默认端口仍为443 | 无 | 不放宽HTTP、私网/保留地址、代理或重定向限制 |
| TEST-PEN-021 | R20 | TASK-PEN-021 | 指定探针代拉订阅 | 拉取探针DTO/持久化、命令请求结果关联、HTTPS/SSRF/端口/大小/哈希/脱敏、Android/离线与超时、控制器和普通/特殊探针全量、Chrome桌面/移动流程 | 已完成 | 认证结果绑定节点；篡改内容拒绝；探针URL不落日志；选择普通Linux/Windows/Mihomo且排除Android；保存载荷含`fetch_node_id`，刷新仅含`subscription_id`；1440/390无溢出和浏览器错误 | 无 | Browser插件不可用，按前端测试技能使用本机Playwright与Chrome；隔离监听已停止，截图位于临时QA目录 |
| TEST-PEN-022 | R21 | TASK-PEN-022 | 页头磁贴快捷入口 | 页面契约测试、Playwright与Chrome桌面1440x1000/移动390x844、真实点击探针管理入口 | 已完成 | 九个名称和href与磁贴面板一致；只有路由管理为`aria-current=page`；点击后进入`/mng/probe`；无溢出、遮挡、控制台或页面错误 | 无 | Browser插件不可用，使用本机Playwright与Chrome；隔离监听已停止，截图位于临时QA目录 |
| TEST-PEN-023 | R20,R21 | TASK-PEN-023 | 可选拉取位置、安全重定向与全管理页快捷切换 | 空`fetch_node_id`保存/调度、主控直接下载、主控/探针跨域HTTPS重定向、私网重定向拒绝、探针在线状态、八页导航契约、控制器/探针全量、Chrome 1440x1000/390x844交互 | 已完成 | 空选由主控拉取；选中仍走认证探针；两端最多5跳且每跳重新校验公网HTTPS；下拉默认主控并标记在线/离线；八页各含9项且唯一高亮；保存再提取载荷准确；无溢出、遮挡、控制台或页面错误 | `go vet ./...`在`probe_node`仍报告既有Windows unsafe.Pointer与mobilecore测试复制锁告警 | Browser插件不可用，使用本机Playwright与Chrome；控制器全量/vet和探针全量通过；隔离监听已停止，截图位于临时QA目录 |
| TEST-PEN-024 | R16,R20 | TASK-PEN-024 | 订阅403兼容回退、物理出口旁路和分级诊断 | 双端固定UA顺序/Accept、最终403提示、探针自定义端口旁路、TUN旁路失败关闭、无TUN不改路由、超时分类、URL/token脱敏、主控/普通/特殊探针全量 | 已完成 | 双端按`clash.meta`、`mihomo/1.19.29`、`Clash.Meta`顺序仅对403重试；旁路目标与固定拨号地址一致；TUN运行时旁路失败不拨号且无TUN不添加旁路；最终错误不含URL | 未向生产订阅发请求，真实服务商是否封锁所选出口IP需发布后复测 | 控制器全量3.204秒、vet通过；并行复跑命中既有端口预留抖动，串行普通探针21.90秒、特殊tag 23.03秒通过 |
...
| DEFECT-PEN-019 | R06,R10,R16 | TEST-PEN-015 | 用户仍需在二次分流页面理解和手工填写订阅请求头，且未配置时Go默认请求标识可能使服务商返回Base64通用订阅而非Clash YAML | 中 | 已完成 | 删除请求头字段、状态和控件；下载端固定设置`User-Agent: clash.meta`及YAML Accept；严格管理DTO拒绝旧字段；专项/全量/vet和桌面/移动Playwright通过 | Base64 URI解析继续作为服务商忽略格式协商时的自动兜底 |
...
| DEFECT-PEN-025 | R20 | TEST-PEN-021 | 提取节点仍由主控HTTP客户端抓取，订阅服务按主控VPN出口返回“请在非VPN网络环境下更新订阅”，无法使用指定探针所在网络 | 高 | 已完成 | 每源持久化`fetch_node_id`；主控通过认证会话下发一次性命令；探针安全下载并回传大小/SHA-256，主控校验后原子应用；无主控回退 | 所选探针自身若仍走VPN，服务商仍可能拒绝 |
| DEFECT-PEN-026 | R20,R21 | TEST-PEN-023 | `fetch_node_id`被服务端强制必填，空选无法保存；探针下载器禁止订阅常见的3xx跳转；快捷入口仅存在路由页 | 高 | 已完成 | 空选显式进入主控安全下载；选中探针保持定向；主控/探针逐跳校验并支持最多5次HTTPS重定向；共享服务端导航渲染器覆盖八个磁贴管理页 | 旧探针版本不识别`subscription_fetch`时仍需先在线升级；选中离线探针会明确失败且不回退主控 |
| DEFECT-PEN-027 | R16,R20 | TEST-PEN-024 | 指定探针已连到订阅服务但返回HTTP 403；下载器仅发送单一客户端标识，且探针固定IP连接未先确保绕过本机VRoute TUN，错误又把所有网络失败压成同一文本 | 高 | 已完成 | 双端403自动回退内置Clash/Mihomo标识；探针IPv4固定拨号前建立物理旁路并在TUN运行时失败关闭；按多IP尝试；超时/拒绝/不可达/TLS/重置分级且不泄漏URL | 若三种标识仍返回403，服务商可能封锁当前拉取出口，需改选真正直连且被服务商接受的探针 |
...
- `nodeStatus/probeReportPayload`：报告`build_kind`以及desired/applied revision/hash、`exit_ready`、健康、版本、会话、字节、错误和`connectivity[target/reachable/latency_ms/error/checked_at]`。
- 探针出口管理接口：`/mng/api/route/special_exits`写入DTO仅接受`node_id/rules`，规则仅含`route_rule_id/target`；响应返回目标下发统计和已分配原规则的只读名称。`/mng/api/probe/node/install`仍由探针管理页生成特殊出口原生/Docker安装信息。
...
- 全局订阅接口：`/mng/api/route/special_exit_subscriptions`维护唯一稳定ID订阅源数组；管理响应逐源只返回configured、`fetch_node_id`、刷新状态和`proxy_options[name/subscription_id/subscription_name]`；管理写入只接受`id/name/enabled/url/fetch_node_id`。`/mng/api/route/special_exits/subscription/refresh`只接受`subscription_id`，并只通知实际引用变化节点的特殊探针。
- 订阅抓取接口：主控经所选探针的认证控制会话发送一次性`subscription_fetch`；双端首选`User-Agent: clash.meta`和接受YAML的`Accept`，仅对403自动尝试内置Mihomo/Clash Meta标识。探针在本地TUN运行时为每个固定IPv4先建立物理出口旁路，按IPv4优先尝试多个公开地址；回传Base64内容、大小、SHA-256或脱敏分级错误；管理页和API不存在手工请求头覆盖或浏览器直拉入口。
...
#### 2.5.2 配置文件
- 主控`probe_route_config.json.special_exit_library`唯一保存订阅URL、每源`fetch_node_id`、全局代理对象和节点到订阅ID映射；`special_exits`只保存每探针`route_rule_id`二级目标、从原规则编译的条目、该探针实际引用的代理对象及revision/hash。管理读取不回显URL或代理秘密，订阅来源和拉取探针不进入特殊出口私有快照。
...
#### 2.5.3 执行报告
- TASK-PEN-000至024全部实现；同一`probe_node`源码形成`normal`和`mihomo_exit`两个产品，特殊版仅Linux amd64且不启动普通本地控制台、代理、系统DNS、同步、DDNS或平台TUN。
- 原`VirtualRouter.RouteRules`是唯一主路由和二次匹配来源，不生成`special-exit:<node_id>`或其他聚合规则；普通探针只见原规则和特殊出口节点，具体Mihomo节点及凭据透明。
- 主控维护一份供所有特殊探针共用、最多32个来源的全局订阅库；每源配置HTTPS URL并可选拉取探针。空选时主控直接安全拉取，选择后只向该非Android探针下发一次性命令且失败不回退主控；两端首选`User-Agent: clash.meta`和YAML `Accept`，仅对403自动尝试内置Mihomo/Clash Meta标识。默认443，显式端口允许1至65535并固定到逐跳校验后的公共解析IP；探针本地TUN运行时固定IPv4先建立物理旁路且失败关闭，多IP按IPv4优先尝试；禁代理/私网/保留地址，最多5次HTTPS重定向，单源8 MiB上限；失败文本和日志不含URL。
...
- 二次分流页包含独立“Clash订阅”和“探针出口”子Tab；全局订阅无需选择探针即可维护并按来源分组展示节点。“探针出口”不自动选探针，每条已指向当前特殊探针的原规则只显示规则名称，并选择DIRECT或“订阅名称 / 节点名称”；匹配条目只在原路由规则界面维护。运行状态只有在desired/applied revision/hash一致且探针/Mihomo健康时显示就绪，并用“出口连通性”磁贴展示实际已选节点的可达性和延迟。
- 八个磁贴管理页通过共享服务端渲染器复用磁贴面板九个可进入页面的名称和路由作为快捷入口；每页仅当前入口使用`aria-current=page`和选中色，桌面置于标题与操作按钮之间，390px移动端两列排列。
...
- 用户不再配置订阅请求头；所选拉取探针自动请求Clash/Mihomo YAML，页面保存载荷的订阅字段严格收敛为`id/name/enabled/url/fetch_node_id`。服务商忽略协商并返回Base64 URI时继续走既有自动兜底。
...
- 本次指定探针代拉整改：控制端`probe_special_exit.go`、`probe_special_exit_mng.go`、`probe_subscription_fetch.go`、`probe_ws.go`、`mng_pages/route.html`及测试；探针端`main.go`、`subscription_fetch.go`及测试；本协作文档。
- 本次可选拉取位置与安全重定向：控制端`probe_special_exit_mng.go`、`probe_subscription_fetch.go`及测试；探针端`subscription_fetch.go`及测试；`mng_pages/route.html`和本协作文档。
- 本次全管理页快捷入口：`mng_pages.go`、八个磁贴管理页模板及对应handlers、`probe_special_exit_test.go`和本协作文档。
...
- Playwright与本机Chrome：真实管理会话打开八个磁贴管理页核对共享导航，再在`/mng/route`点击二次分流，采集桌面/移动DOM、控制台与截图。
...
- TEST-PEN-018：控制器全量及vet；`cd probe_node; go test ./... -count=1; go test -tags mihomo_exit ./... -count=1`；`TestRouteConfigSyncControlReportsAppliedStateImmediately`验证控制帧同步完成后调用即时报告器；Playwright验证`dispatched=1`后轮询applied revision/hash并显示“已保存并同步”。
- TEST-PEN-019：`TestGlobalClashLibraryIsSharedAndSnapshotsContainOnlySelectedNodes`、逐源刷新/重名/last-known-good、状态来源/延迟专项；`TestSelectedProbeMihomoConnectivityTargetsDeduplicatesAndSkipsDirect`和`TestMeasureProbeMihomoConnectivityUsesDelayAPI`；Node解析页面脚本；隔离控制器Playwright与Edge验证全局子Tab、逐行提取、订阅分组、“订阅 / 节点”下拉、连通性磁贴及1440x1000/390x844布局。
- TEST-PEN-021：控制端定向用例覆盖拉取探针DTO/持久化、Android/不存在探针拒绝、离线会话、认证结果节点绑定、Base64/大小/SHA-256篡改拒绝及原子刷新；探针定向用例覆盖控制消息分派、HTTPS、自定义端口、公开IP固定拨号、私网/HTTP拒绝和URL脱敏；随后执行控制器全量/vet、普通探针全量、特殊tag全量；隔离控制器经Playwright与Chrome验证每源拉取探针选择、精确保存/刷新载荷、1440x1000与390x844无溢出及零浏览器错误。
- TEST-PEN-022：`go test ./internal/core -run "TestMngRoutePageIncludes(SpecialExitWorkflow|TileShortcuts)" -count=1`；隔离控制器经Playwright与Chrome验证九个磁贴链接、路由管理选中态、点击“探针管理”进入`/mng/probe`，并采集1440x1000与390x844布局、控制台和截图。
- TEST-PEN-023：控制器专项覆盖空拉取探针保存、主控调度、主控HTTPS安全重定向和私网重定向拒绝；探针专项覆盖逐跳HTTPS安全重定向；执行控制器`go test ./... -count=1`与`go vet ./...`、探针`go test ./... -count=1`。隔离控制器经Playwright与Chrome验证主控默认项、节点在线/离线文案、八个磁贴管理页九项快捷入口及唯一当前态、保存后提取载荷和1440x1000/390x844布局，控制台与页面错误均为空；隔离进程已停止且15030端口清空。
...
- 历史TEST-PEN-012阶段的旧载荷和五区页面证据已由TEST-PEN-016至021取代；当前探针出口载荷仅`node_id/rules`，全局订阅载荷仅`subscriptions[id/name/enabled/url/fetch_node_id]`。
...
- 通过：TEST-PEN-020非443 HTTPS订阅端口；探针侧8443校验及固定公共IP拨号通过，0和65536拒绝；普通/特殊探针全量通过。
- 通过：TEST-PEN-021指定探针代拉；控制器全量2.453秒与vet通过，普通探针全量21.634秒、特殊tag全量20.367秒通过。认证结果关联、身份/大小/哈希校验、Android/离线/SSRF/超时/脱敏边界均有定向正负向测试。
- 通过：控制器与`mihomo_exit`探针均以`GOOS=linux GOARCH=amd64 CGO_ENABLED=0`完成交叉构建，指定探针代拉实现可进入目标Linux x64发布物。
- 通过：Playwright与Chrome在1440x1000和390x844验证每个订阅独立选择普通Linux、普通Windows或Mihomo拉取探针，Android不进入候选；保存字段精确含`fetch_node_id`，刷新载荷精确只有`subscription_id`；无横向溢出、控制台或页面错误，截图位于临时QA目录，隔离控制器已停止且15030端口清空。Browser插件在本任务不可用，因此按前端测试技能使用本机Playwright。
- 通过：TEST-PEN-022页头快捷入口契约与浏览器流程；九个入口名称/href准确，路由管理是唯一当前页项，点击探针管理成功进入`/mng/probe`；1440x1000填充页头中部，390x844两列显示，无横向溢出、遮挡、控制台或页面错误，隔离控制器已停止且15030端口清空。
- 通过：TEST-PEN-023可选拉取位置、安全重定向和全管理页导航；控制器全量2.677秒与vet通过，探针全量21.631秒通过。浏览器确认空选主控、指定探针在线状态、八页唯一高亮、保存/提取载荷和桌面/移动布局，无溢出、遮挡、控制台或页面错误；探针vet仅剩既有Windows unsafe.Pointer和mobilecore测试复制锁告警。
- 通过：TEST-PEN-024订阅403兼容回退、物理出口旁路和分级诊断；双端专项确认固定UA顺序、Accept、403提示和URL/token脱敏，探针专项确认8443旁路地址与拨号一致、TUN旁路失败关闭、无TUN不改路由及超时分类；控制器全量3.204秒与vet通过。普通/特殊探针并行全量各命中一次既有TCP/UDP端口预留抖动，随后串行普通探针21.90秒、特殊tag 23.03秒完整通过。
...
- 指定拉取探针必须在线并已升级到支持`subscription_fetch`及物理出口旁路的版本；旧版探针会在主控等待窗口后明确超时，或可能通过本机TUN拉取。新版本自动尝试内置客户端标识后若仍返回403，说明服务商仍可能封锁该出口IP或请求特征；此时应为该来源改选实际直连且被服务商接受的探针。
...
- 指定探针代拉可通过回滚`fetch_node_id`模型、`subscription_fetch`控制消息和页面选择器撤销；不得恢复浏览器直拉或主控出口静默回退，旧配置中的附加字段会被旧主控忽略。
- 持久数据回滚前备份`data/`和`log/`；`temp/`可直接重建。

#### 2.5.10 结论
- TASK-PEN-000至024及DEFECT-PEN-009至027代码整改全部完成；原路由规则唯一来源、全局Clash订阅库、主控/指定探针可选拉取、安全订阅重定向、403兼容回退、探针物理出口旁路、全部磁贴管理页快捷入口、逐规则DIRECT/具体节点选择、目标同步与即时回报、v3快照裁剪、订阅格式/Reality过滤和出口连通性均闭合，本地专项、全量和桌面/移动浏览器证据通过，Architect增量门禁已放行。
...
|---|---|---|---|---|---|---|---|
| 无 | TASK-PEN-000至024 | 无 | 无未处理任务包缺口或接口冲突 | 无 | 进入最终门禁 | 已完成 | 无需整改 |
