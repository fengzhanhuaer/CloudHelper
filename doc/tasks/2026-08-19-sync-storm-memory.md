...
| REQ-005 | 完成回归与远端复核 | controller、probe normal、router tag 测试通过；远端 Swap 和当前资源状态复核完成 | 高 | `已完成` | 用户要求修复排查 |
| REQ-006 | 发布并更新测试环境 | release workflow 成功、资产齐全；netcup2o 通过在线升级运行新版本并完成实机验证 | 高 | `已完成` | 用户最新指令 |
...
- 当前计划步骤：全部完成
- 当前门禁：完成门禁通过
- 最近完成检查点：v0.4.7 已发布并通过在线升级运行于 netcup2o；14:15:00 至 14:17:42 的两个完整分钟窗口内 exit node 新增同步请求为 0，三个服务 active，无新增 OOM，主控内存约 67 至 70 MiB。
- 工作区状态：分支 `mapledev` 与远端自动版本提交一致；仅本账本最终状态待 `[skip release]` 提交。
- 下一步唯一动作：无。
...
| TASK-006 | 提交、发布、在线升级测试环境并验证 | `已完成` | REQ-006 | Git、GitHub Actions、netcup2o 在线升级 | v0.4.7 发布成功且实机新版本稳定 |
...
| controller `probe_command.go`、`probe_ws.go` | 限频记录路由同步分发调用方和 Linux router 报告触发节点 | 在不放大日志压力的前提下定位 v0.4.5 遗留触发链 | REQ-003, REQ-006 / TASK-006 | TEST-005, TEST-007 | RB-002 |
| controller `probe_runtime.go`、`probe_linux_router_test.go` | 基础心跳原位更新 runtime 基础字段，保留 BuildKind、LinuxRouter 和 SpecialExit 产品状态 | 防止每次心跳先清空产品状态、再把相同 #23 报告误判为首次变化 | REQ-002, REQ-003 / TASK-003 | TEST-002, TEST-005, TEST-007 | RB-002 |
| controller 临时诊断埋点 | 最终版移除同步来源限频日志与对应测试 | 来源已确认，避免保留长期调试日志 | REQ-006 / TASK-006 | TEST-005 | RB-002 |
...
| TEST-007 | 发布与在线升级实机验证 | REQ-006 / TASK-006 | release workflow、资产核对、在线升级、服务/内存/同步频率观察 | 新版本运行且无同步风暴、无新增 OOM | v0.4.7 工作流 32251242405 全绿、10 项资产；主控在线升级成功；两个完整分钟窗口新增同步 0；无新增 OOM | `已完成` | Actions 与 netcup2o 14:15 至 14:18 CEST 日志 |

### 5.2 未执行测试

Go race detector因当前 `CGO_ENABLED=0` 未执行；以并发定向测试、100 轮根因测试、完整测试、vet、跨平台发布构建和实机观察替代。未手工部署或替换运行时二进制。
...
| REQ-006 | 发布并在线升级测试环境 | 全部 | TASK-006 | release workflow、netcup2o | TEST-007 | v0.4.7 全矩阵发布、在线升级与实机稳定窗口通过 | `已完成` |
...
| DEF-004 | REQ-002, REQ-003 / TEST-002, TEST-007 | 严重 | 基础心跳整体覆盖 runtime 记录，清空 BuildKind/LinuxRouter；同一消息随后处理 #23 产品报告时永远从零值比较，固定触发全网同步 | 已修复并实机验证 | controller `probe_runtime.go` | v0.4.6 来源日志、根因回归 100 轮、v0.4.7 两分钟新增同步 0 |
...
| FACT-011 | v0.4.5 忽略 revision/SHA 后同步仍持续，且 #23 在线报告的代理开关、CIDR、ACL 在 50 秒多轮采样中稳定 | netcup2o runtime API 与 exit runtime log | revision/SHA 并非完整根因，需要记录精确分发调用方 |
| FACT-012 | v0.4.6 每次同步来源均为 `schedule:linux_router_report node_id=23`，间隔约 10 至 15 秒 | netcup2o controller 来源日志 | #23 是触发节点 |
| FACT-013 | `updateProbeRuntimeReportWithPlatform` 在产品处理前整体重建同一节点记录，丢失 BuildKind/LinuxRouter | controller 消息顺序、源码与根因回归测试 | 已确认同步风暴最终根因 |

## 十一、风险与阻塞

| 编号 | 类型 | 描述与证据 | 影响 | 缓解或所需动作 | 状态 |
|---|---|---|---|---|---|
| RISK-001 | 线上验证 | 实机长期内存曲线需发布与在线升级后观察 | 短期实机窗口已通过 | 两分钟同步为 0、内存稳定、无新 OOM；后续由现有监控持续观察 | 已关闭 |
| RISK-002 | 容量 | cloudflared 当前曾达到约 500 MiB | controller 停止时仍可能内存紧张 | 2GB Swap 已缓冲；cloudflared 属独立后续容量事项 | 已接受 |
| RISK-003 | 测试环境 | Go race detector 要求 CGO，但当前 `CGO_ENABLED=0` 且无已确认 C 编译器；probe vet 还有既有 mobilecore 锁复制与 Windows unsafe 告警 | 未获得动态数据竞争检测，vet 非全绿 | 以锁保护、定向并发 20 轮、三套全测、100 轮根因测试、交叉编译和实机观察替代 | 已接受 |
...
| 检查项 | 结论 | 证据或条件 |
|---|---|---|
| 用户最新目标和有效需求逐项验收 | 通过 | REQ-001 至 REQ-006 全部完成 |
| 端到端追踪闭合 | 通过 | 第六章全部已完成 |
| 测试已执行或缺口影响已准确记录 | 通过 | TEST-001 至 TEST-007；race 缺口已记录 |
| 缺陷已关闭或成为用户接受的遗留风险 | 通过 | DEF-001 至 DEF-004 全部关闭 |
| 决策、冲突、回滚、风险和阻塞状态已更新 | 通过 | 第七、九、十一章 |
| 最终差异无范围漂移、无关回退和调试残留 | 通过 | 临时来源日志已从 v0.4.7 移除 |
| 账本与工作区一致，下一步唯一动作为“无” | 通过 | 最终文档提交后无工作区差异 |
...
| 2026-08-19 | 发布并升级 v0.4.5，采样 #22/#23 | 同步仍持续；#23 在线但路由语义字段稳定，#22 离线且状态稳定 | 推翻“revision/SHA 是最终根因”，增加限频来源日志 | 发布诊断版并捕获真实调用入口 |
| 2026-08-19 | 发布并升级 v0.4.6，捕获同步来源 | 所有事件均由 #23 心跳报告触发；基础心跳先清空产品状态才是最终根因 | 实现保留产品状态的原位更新并移除诊断埋点 | 发布最终修复并在线升级观察 |
| 2026-08-19 | 发布并在线升级 v0.4.7 | 14:15 至 14:17 两分钟新增同步 0，主控约 67 至 70 MiB，三个服务 active，无新增 OOM | REQ-001 至 REQ-006 全部验收 | 无 |

## 十四、完成摘要

- 交付结果：2GB Swap、双侧有界同步、掉线不广播、心跳保留产品状态均已交付；v0.4.7 已发布并在线运行。
- 需求验收：REQ-001 至 REQ-006 全部通过。
- 测试结论：本地全测、静态检查、发布矩阵、根因测试 100 轮及实机稳定窗口通过。
- 缺陷与风险：DEF-001 至 DEF-004 已关闭；独立 cloudflared 容量与 race 环境缺口记录为已接受风险。
