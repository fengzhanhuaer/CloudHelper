...
var mobilePublicIPCollector = newPublicIPCollector()
...
func collectCommandIPs() ([]string, []string) {
	...
	return parseIPAddrOutput(string(out))
}

func parseIPAddrOutput(raw string) ([]string, []string) {
		...
		for i, field := range fields {
			...
			if field == "inet" {
				if ip4 := ip.To4(); ip4 != nil {
					seen4[ip4.String()] = struct{}{}
				}
				continue
			}
			if ip.To16() != nil && ip.To4() == nil {
				seen6[ip.String()] = struct{}{}
			}
		}
	}
	return sortedStringKeys(seen4), sortedStringKeys(seen6)
}
...
func collectPublicIPs() ([]string, []string) {
	...
	return mobilePublicIPCollector.collect()
}
...
func sniffPublicIPs() ([]string, []string, bool) {
	...
	ipv4 := sortedStringKeys(ipv4Set)
	ipv6 := sortedStringKeys(ipv6Set)
...
func fetchPublicIP(endpoint string, network string) string {
	...
	defer cancel()
	dialer := &net.Dialer{Timeout: publicIPRequestTimeout, KeepAlive: 15 * time.Second}
	transport := &http.Transport{
		Proxy:               http.ProxyFromEnvironment,
		DialContext:         forceNetworkDialContext(dialer, network),
		TLSHandshakeTimeout: publicIPRequestTimeout,
	}
	client := &http.Client{Transport: transport, Timeout: publicIPRequestTimeout}
	...
	return normalizePublicIPValue(string(body), network)
}

func forceNetworkDialContext(dialer *net.Dialer, forceNetwork string) func(ctx context.Context, network string, address string) (net.Conn, error) {
