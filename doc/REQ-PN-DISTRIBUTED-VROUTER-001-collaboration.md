...
- 每条拓扑规则可以独立配置两端探针的服务域名与服务端口；端口默认 `12040`，允许在多条规则中复用，域名候选来自 Cloudflare DDNS 模块，同时允许填写内网域名；服务域名、端口和 A/B 节点属于可变配置，不参与隐藏 `vrouter-*` 规则身份生成。规则 ID 在首次保存时按 `vr-N` 序号初始化一次，后续 `chain_id` 只由规则 ID 派生。
...
- RQ-DVRT-018: 每条拓扑规则可以独立配置 A/B 两端服务域名与服务端口，端口默认 `12040` 且允许复用；域名从 Cloudflare DDNS 模块选择或手动填写内网域名，配置独立保存在虚拟路由器拓扑规则内，不混用旧探针侧配置；规则 ID 必须在首次保存时按 `vr-N` 序号初始化一次，隐藏 `vrouter-*` 的 `chain_id` 必须仅基于规则 ID 派生，不能因为 A/B 节点、服务域名或端口调整而变化。
...
- RQ-DVRT-023: probe_node 应在应用虚拟路由配置时构建运行态拓扑索引，至少包含全部探针 `node_id -> virtual_ip`、`virtual_ip -> node_id`、拓扑规则 ID 映射和邻接图；本机虚拟 IP 也必须作为运行态状态保存。数据面判断包是否发给本机必须直接比较目标虚拟 IP 与本机虚拟 IP，`node_id` 只用于控制面初始化和查询 IP 映射，不得作为数据面收包判据。
...
- 主控探针同步接口 `/api/probe/link/config/grouped` 兼容扩展返回虚拟路由器配置，旧探针忽略新增字段；虚拟路由器配置必须向每个探针下发全量探针虚拟 IP 映射和全量拓扑规则，不能只下发与当前节点直接相关的子图。
- probe_node 同步到虚拟路由器配置后，保存本地缓存，并为本探针建立静态局域网 IP 与可达拓扑视图。
- probe_node 应把完整虚拟拓扑编译为本地运行态索引，包括全量探针虚拟 IP 映射、规则映射和邻接关系；本探针虚拟 IP 保存为运行态状态。虚拟路由数据面以 `packet.dst_ip == local_virtual_ip` 判断本机收包，路由、探测和排障以运行态索引查询目标 IP、目标节点、规则和下一跳，避免用 path 末端、chain_id 或物理方向误判路过包。
...
- 隐藏 `vrouter-*` 运行态复用现有共享监听端口与 `chain_id` 分发机制，因此多条拓扑规则可以复用默认 `12040` 端口；`chain_id` 表示规则身份，只由首次初始化的规则 ID 派生，服务域名、服务端口和 A/B 节点只是拨号/监听与拓扑参数，不参与 `chain_id` 生成，避免配置变更时 A/B 两端因同步先后出现 `runtime_not_found`。
...
- 隐藏 `vrouter-*` 的 `chain_id` 必须稳定绑定首次初始化的规则 ID，不能绑定可变 A/B 节点、服务域名或服务端口；同端口复用依赖 shared relay 按稳定 `chain_id` 分发。
