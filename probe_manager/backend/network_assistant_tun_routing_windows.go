...
func ensureWindowsTUNAdapterIPv4Routing() (windowsAdapterInfo, error) {
	...
	if err := windowsEnsureInterfaceIPv4Address(adapter.InterfaceIndex, tunRouteIPv4Address, tunRouteIPv4PrefixLength); err != nil {
		...
		for attempt, delay := range retryDelays {
			attemptNo := attempt + 1
			log.Printf("[network-assistant] tun ipv4 bind retry begin (no adapter reset): attempt=%d/%d if=%d delay_ms=%d", attemptNo, len(retryDelays), adapter.InterfaceIndex, delay/time.Millisecond)
			time.Sleep(delay)
