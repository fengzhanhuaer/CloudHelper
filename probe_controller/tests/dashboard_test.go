...
func TestDashboardNetworkRouteNoAuthRequired(t *testing.T) {
	...
	if len(payload.Items) != 1 || len(payload.Items[0].Points) != 1 {
		t.Fatalf("unexpected network payload: %+v", payload)
	}
	if payload.Items[0].NodeNo != 1 {
		t.Fatalf("node_no=%d want 1", payload.Items[0].NodeNo)
	}
	if payload.Items[0].Points[0].LatencyAvgMS != 23.5 || payload.Items[0].Points[0].LossPercent != 1.5 {
		t.Fatalf("point=%+v", payload.Items[0].Points[0])
	}
}
