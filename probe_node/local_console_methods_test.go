...
func TestProbeLocalAPIMethodGuards(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "dns clear should only allow POST", method: http.MethodGet, path: "/local/api/dns/clear", body: nil},
		...
		{name: "proxy explicit enable should only allow POST", method: http.MethodGet, path: "/local/api/proxy/explicit/enable", body: nil},
		{name: "proxy explicit direct should only allow POST", method: http.MethodGet, path: "/local/api/proxy/explicit/direct", body: nil},
		...
		{name: "proxy monitor should only allow GET", method: http.MethodPost, path: "/local/api/proxy/monitor", body: map[string]any{}},
		{name: "proxy substreams should only allow GET", method: http.MethodPost, path: "/local/api/proxy/substreams", body: map[string]any{}},
		...
		{name: "proxy chains refresh should only allow POST", method: http.MethodGet, path: "/local/api/proxy/chains/refresh", body: nil},
		...
		{name: "proxy groups restore should only allow POST", method: http.MethodGet, path: "/local/api/proxy/groups/restore", body: nil},
	}
