...
func TestProbeRouteEgressDialNetwork(t *testing.T) {
	cases := []struct {
		name   string
		base   string
		target string
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			if got := probeRouteEgressDialNetwork(tt.base, tt.target); got != tt.want {
				t.Fatalf("probeRouteEgressDialNetwork(%q,%q)=%q want %q", tt.base, tt.target, got, tt.want)
			}
		})
	}
}

func TestApplyProbeRouteEgressDialerDoesNotBindInterface(t *testing.T) {
	// Egress bypass is route-based; the dialer must not install a per-socket
	// interface binding hook.
	d := applyProbeRouteEgressDialer(&net.Dialer{})
	if d.Control != nil {
