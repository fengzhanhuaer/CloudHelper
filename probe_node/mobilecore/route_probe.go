package mobilecore

import (
	"bytes"
	"context"
	"crypto/hmac"
	"crypto/sha256"
	"crypto/tls"
	"encoding/hex"
	"encoding/json"
	"errors"
	"fmt"
	"io"
	"net"
	"net/http"
	"net/url"
	"sort"
	"strconv"
	"strings"
	"sync"
	"time"

	"github.com/gorilla/websocket"
	"github.com/quic-go/quic-go"
	"github.com/quic-go/quic-go/http3"
)

const (
	routeRelayAPIPath                  = "/api/node/route/relay"
	routeLegacyRouteIDHeader           = "X-CH-Route-ID"
	routeCodexRouteIDHeader            = "X-Codex-Route-Id"
	linkCodexAuthModeHeader            = "X-Codex-Auth-Mode"
	linkCodexMACHeader                 = "X-Codex-Mac"
	linkCodexAuthTicketHeader          = "X-Codex-User-Auth-Ticket"
	linkCodexVersionHeader             = "X-Codex-Api-Version"
	linkCodexRelayModeHeader           = "X-Codex-Relay-Mode"
	linkCodexRelayRoleHeader           = "X-Codex-Relay-Role"
	linkCodexSpeedBytesHeader          = "X-Codex-Speed-Bytes"
	linkAuthPacketVersion              = "2025-03-22"
	routeRelayModeBridge               = "bridge"
	routeRelayModeStream               = "stream"
	routeRelayModeSpeedTest            = "speed_test"
	routeRelayModeSpeedDebug           = "speed_debug"
	routeRelayModePingPong             = "ping_pong"
	linkBridgeRoleToNext               = "to_next"
	routeRelayProtocolProbeTimeout     = 6 * time.Second
	linkPortForwardResponseReadTimeout = 10 * time.Second
	routeRelaySpeedTestBytes           = 128 * 1024 * 1024
	routeRelaySpeedTestMaxBytes        = 256 * 1024 * 1024
	routeRelaySpeedTestTimeout         = 10 * time.Second
	routeRelayWebSocketBufferBytes     = 512 * 1024
	routeRelayQUICInitialStreamWindow  = 128 * 1024 * 1024
	routeRelayQUICMaxStreamWindow      = 512 * 1024 * 1024
	routeRelayQUICInitialConnWindow    = 512 * 1024 * 1024
	routeRelayQUICMaxConnWindow        = 1024 * 1024 * 1024
)

type routeServerItem struct {
	RouteID         string         `json:"route_id"`
	RelayRouteID    string         `json:"relay_route_id"`
	ClientEntryID   string         `json:"client_entry_id"`
	ClientEntryType string         `json:"client_entry_type"`
	RouteType       string         `json:"route_type"`
	Name            string         `json:"name"`
	UserID          string         `json:"user_id"`
	UserPublicKey   string         `json:"user_public_key"`
	Secret          string         `json:"secret"`
	AuthTicket      string         `json:"auth_ticket,omitempty"`
	EntryNodeID     string         `json:"entry_node_id"`
	ExitNodeID      string         `json:"exit_node_id"`
	CascadeNodeIDs  []string       `json:"cascade_node_ids"`
	RouteLayer      string         `json:"route_layer"`
	HopConfigs      []routeHopItem `json:"hop_configs"`
	EgressHost      string         `json:"egress_host"`
	EgressPort      int            `json:"egress_port"`
}

type routeHopItem struct {
	NodeNo       int    `json:"node_no"`
	ListenHost   string `json:"listen_host"`
	ListenPort   int    `json:"listen_port"`
	ExternalPort int    `json:"external_port"`
	RouteLayer   string `json:"route_layer"`
	DialMode     string `json:"dial_mode"`
	RelayHost    string `json:"relay_host"`
}

type routeEndpoint struct {
	RouteID             string `json:"route_id"`
	RouteName           string `json:"route_name,omitempty"`
	EntryNodeID         string `json:"entry_node_id,omitempty"`
	EntryHost           string `json:"entry_host,omitempty"`
	EntryPort           int    `json:"entry_port,omitempty"`
	RouteLayer          string `json:"route_layer,omitempty"`
	RouteSecret         string `json:"-"`
	AuthTicket          string `json:"-"`
	PreserveRelayDomain bool   `json:"-"`
}

type routeStatusItem struct {
	RouteID       string `json:"route_id"`
	RelayRouteID  string `json:"relay_route_id,omitempty"`
	ClientEntryID string `json:"client_entry_id,omitempty"`
	RouteName     string `json:"route_name,omitempty"`
	RouteType     string `json:"route_type,omitempty"`
	Status        string `json:"status"`
	EntryNodeID   string `json:"entry_node_id,omitempty"`
	EntryHost     string `json:"entry_host,omitempty"`
	EntryPort     int    `json:"entry_port,omitempty"`
	RouteLayer    string `json:"route_layer,omitempty"`
	Error         string `json:"error,omitempty"`
}

type routeReachabilityResult struct {
	Protocol  string `json:"protocol"`
	OK        bool   `json:"ok"`
	LatencyMS int64  `json:"latency_ms,omitempty"`
	Error     string `json:"error,omitempty"`
}

type routeSpeedTestResult struct {
	Protocol         string `json:"protocol"`
	OK               bool   `json:"ok"`
	LatencyMS        int64  `json:"latency_ms,omitempty"`
	Bytes            int64  `json:"bytes,omitempty"`
	RequestedBytes   int64  `json:"requested_bytes,omitempty"`
	DurationMS       int64  `json:"duration_ms,omitempty"`
	RateBPS          int64  `json:"rate_bps,omitempty"`
	ReadCalls        int64  `json:"read_calls,omitempty"`
	ReadChunkBytes   int64  `json:"read_chunk_bytes,omitempty"`
	AvgReadBytes     int64  `json:"avg_read_bytes,omitempty"`
	FirstByteMS      int64  `json:"first_byte_ms,omitempty"`
	TotalReadBlockMS int64  `json:"total_read_block_ms,omitempty"`
	MaxReadBlockMS   int64  `json:"max_read_block_ms,omitempty"`
	LastReadBlockMS  int64  `json:"last_read_block_ms,omitempty"`
	OpenHandshakeMS  int64  `json:"open_handshake_ms,omitempty"`
	LocalStartedAt   string `json:"local_started_at,omitempty"`
	LocalFirstByteAt string `json:"local_first_byte_at,omitempty"`
	LocalCompletedAt string `json:"local_completed_at,omitempty"`
	Error            string `json:"error,omitempty"`
	StartedAt        string `json:"started_at,omitempty"`
	EndedAt          string `json:"ended_at,omitempty"`
}

type routeSpeedDebugItemPayload struct {
	ID                 string `json:"id"`
	RouteID            string `json:"route_id,omitempty"`
	Role               string `json:"role,omitempty"`
	Side               string `json:"side,omitempty"`
	Transport          string `json:"transport,omitempty"`
	RemoteAddr         string `json:"remote_addr,omitempty"`
	Status             string `json:"status,omitempty"`
	Error              string `json:"error,omitempty"`
	RequestedBytes     int64  `json:"requested_bytes,omitempty"`
	ChunkBytes         int64  `json:"chunk_bytes,omitempty"`
	Bytes              int64  `json:"bytes,omitempty"`
	RemainingBytes     int64  `json:"remaining_bytes,omitempty"`
	RateBPS            int64  `json:"rate_bps,omitempty"`
	WriteCalls         int64  `json:"write_calls,omitempty"`
	TotalWriteBlockMS  int64  `json:"total_write_block_ms,omitempty"`
	MaxWriteBlockMS    int64  `json:"max_write_block_ms,omitempty"`
	LastWriteBlockMS   int64  `json:"last_write_block_ms,omitempty"`
	LastWriteBlockedAt string `json:"last_write_blocked_at,omitempty"`
	StartedAt          string `json:"started_at,omitempty"`
	UpdatedAt          string `json:"updated_at,omitempty"`
	EndedAt            string `json:"ended_at,omitempty"`
	AgeMS              int64  `json:"age_ms,omitempty"`
	DurationMS         int64  `json:"duration_ms,omitempty"`
}

type routeSpeedDebugResultPayload struct {
	Type      string                       `json:"type"`
	RequestID string                       `json:"request_id,omitempty"`
	NodeID    string                       `json:"node_id,omitempty"`
	OK        bool                         `json:"ok"`
	Scope     string                       `json:"scope,omitempty"`
	FetchedAt string                       `json:"fetched_at,omitempty"`
	Active    []routeSpeedDebugItemPayload `json:"active"`
	Recent    []routeSpeedDebugItemPayload `json:"recent"`
	Error     string                       `json:"error,omitempty"`
}

type routeTunnelOpenRequest struct {
	Type             string                  `json:"type"`
	Network          string                  `json:"network,omitempty"`
	Address          string                  `json:"address,omitempty"`
	FlowID           string                  `json:"flow_id,omitempty"`
	ResumeToken      string                  `json:"resume_token,omitempty"`
	ResumeEpoch      uint64                  `json:"resume_epoch,omitempty"`
	ReadOffset       uint64                  `json:"read_offset,omitempty"`
	WriteOffset      uint64                  `json:"write_offset,omitempty"`
	AppProtocol      string                  `json:"app_protocol,omitempty"`
	Priority         string                  `json:"priority,omitempty"`
	ResumePolicy     string                  `json:"resume_policy,omitempty"`
	LatencySensitive bool                    `json:"latency_sensitive,omitempty"`
	AssociationV2    *routeAssociationV2Meta `json:"association_v2,omitempty"`
	SpeedBytes       int64                   `json:"speed_bytes,omitempty"`
	PingBytes        int64                   `json:"ping_bytes,omitempty"`
}

type routeAssociationV2Meta struct {
	Version          int    `json:"version"`
	Transport        string `json:"transport,omitempty"`
	RouteGroup       string `json:"route_group,omitempty"`
	RouteNodeID      string `json:"route_node_id,omitempty"`
	RouteTarget      string `json:"route_target,omitempty"`
	RouteFingerprint string `json:"route_fingerprint,omitempty"`
	NATMode          string `json:"nat_mode,omitempty"`
	TTLProfile       string `json:"ttl_profile,omitempty"`
	IdleTimeoutMS    int64  `json:"idle_timeout_ms,omitempty"`
	GCIntervalMS     int64  `json:"gc_interval_ms,omitempty"`
	CreatedAtUnixMS  int64  `json:"created_at_unix_ms,omitempty"`
	AssocKeyV2       string `json:"assoc_key_v2,omitempty"`
	FlowID           string `json:"flow_id,omitempty"`
	SrcIP            string `json:"src_ip,omitempty"`
	SrcPort          uint16 `json:"src_port,omitempty"`
	DstIP            string `json:"dst_ip,omitempty"`
	DstPort          uint16 `json:"dst_port,omitempty"`
	IPFamily         int    `json:"ip_family,omitempty"`
	SourceKey        string `json:"source_key,omitempty"`
	SourceRefs       int64  `json:"source_refs,omitempty"`
}

type routeTunnelOpenResponse struct {
	OK          bool   `json:"ok"`
	Error       string `json:"error,omitempty"`
	FlowID      string `json:"flow_id,omitempty"`
	ResumeToken string `json:"resume_token,omitempty"`
	ResumeEpoch uint64 `json:"resume_epoch,omitempty"`
	ReadOffset  uint64 `json:"read_offset,omitempty"`
	WriteOffset uint64 `json:"write_offset,omitempty"`
}

// RouteStatus returns the Android-visible route endpoint inventory.
func RouteStatus(configDir string) string {
	items, err := loadRoutes(configDir)
	if err != nil {
		return marshalRouteJSON(map[string]any{
			"ok":         false,
			"error":      err.Error(),
			"routes":     []routeStatusItem{},
			"updated_at": time.Now().UTC().Format(time.RFC3339),
		})
	}
	out := make([]routeStatusItem, 0, len(items))
	for _, item := range items {
		status := routeStatusItem{
			RouteID:       strings.TrimSpace(item.RouteID),
			RelayRouteID:  strings.TrimSpace(item.RelayRouteID),
			ClientEntryID: strings.TrimSpace(item.ClientEntryID),
			RouteName:     strings.TrimSpace(item.Name),
			RouteType:     strings.TrimSpace(item.RouteType),
			Status:        "configured",
		}
		endpoint, err := resolveRouteEndpoint(item)
		if err != nil {
			status.Status = "unconfigured"
			status.Error = err.Error()
		} else {
			status.EntryNodeID = endpoint.EntryNodeID
			status.EntryHost = endpoint.EntryHost
			status.EntryPort = endpoint.EntryPort
			status.RouteLayer = endpoint.RouteLayer
		}
		out = append(out, status)
	}
	return marshalRouteJSON(map[string]any{
		"ok":         true,
		"routes":     out,
		"count":      len(out),
		"updated_at": time.Now().UTC().Format(time.RFC3339),
	})
}

// LinkLatency runs the same relay ping-pong probe used by the desktop probe node.
func LinkLatency(configDir string, routeID string) string {
	item, endpoint, err := loadRouteEndpointByID(configDir, routeID)
	if err != nil {
		return marshalRouteJSON(map[string]any{
			"ok":         false,
			"route_id":   strings.TrimSpace(routeID),
			"status":     "unconfigured",
			"error":      err.Error(),
			"updated_at": time.Now().UTC().Format(time.RFC3339),
		})
	}
	protocols := routeReachabilityProtocolsForEndpoint(item, endpoint)
	resultsCh := make(chan routeReachabilityResult, len(protocols))
	for _, protocol := range protocols {
		go func(protocol string) {
			latency, err := linkPingPongProbe(endpoint, protocol)
			result := routeReachabilityResult{
				Protocol:  normalizeRouteLayer(protocol),
				OK:        err == nil,
				LatencyMS: durationMilliseconds(latency),
			}
			if err != nil {
				result.Error = strings.TrimSpace(err.Error())
				result.LatencyMS = 0
			}
			resultsCh <- result
		}(protocol)
	}
	results := make([]routeReachabilityResult, 0, len(protocols))
	reachableCount := 0
	bestProtocol := ""
	bestLatencyMS := int64(0)
	for range protocols {
		result := <-resultsCh
		results = append(results, result)
		if !result.OK {
			continue
		}
		reachableCount++
		if bestProtocol == "" || bestLatencyMS <= 0 || (result.LatencyMS > 0 && result.LatencyMS < bestLatencyMS) || (result.LatencyMS == bestLatencyMS && linkProtocolOrder(result.Protocol) < linkProtocolOrder(bestProtocol)) {
			bestProtocol = normalizeRouteLayer(result.Protocol)
			bestLatencyMS = result.LatencyMS
		}
	}
	sort.Slice(results, func(i, j int) bool {
		return linkProtocolOrder(results[i].Protocol) < linkProtocolOrder(results[j].Protocol)
	})
	base := map[string]any{
		"ok":              bestProtocol != "",
		"route_id":        strings.TrimSpace(item.RouteID),
		"route_name":      strings.TrimSpace(item.Name),
		"status":          "reachable",
		"latency_ms":      bestLatencyMS,
		"reachable_count": reachableCount,
		"tested_count":    len(results),
		"best_protocol":   bestProtocol,
		"entry_host":      endpoint.EntryHost,
		"entry_port":      endpoint.EntryPort,
		"route_layer":     endpoint.RouteLayer,
		"results":         results,
		"updated_at":      time.Now().UTC().Format(time.RFC3339),
	}
	if bestProtocol == "" {
		base["status"] = "unreachable"
		base["error"] = "all relay protocols are unreachable"
	}
	return marshalRouteJSON(base)
}

// RouteSpeed runs the real relay speed_test mode and consumes server-sent bytes.
func RouteSpeed(configDir string, routeID string, protocol string) string {
	item, endpoint, err := loadRouteEndpointByID(configDir, routeID)
	if err != nil {
		return marshalRouteJSON(map[string]any{
			"ok":         false,
			"route_id":   strings.TrimSpace(routeID),
			"status":     "unconfigured",
			"error":      err.Error(),
			"updated_at": time.Now().UTC().Format(time.RFC3339),
		})
	}
	cleanProtocol := ""
	if strings.TrimSpace(protocol) != "" {
		cleanProtocol = normalizeRouteLayer(protocol)
		if !isRouteRelaySupportedProtocol(cleanProtocol) {
			return marshalRouteJSON(map[string]any{"ok": false, "error": "protocol must be websocket-h3 or websocket"})
		}
	}
	if isRouteCFEntry(item) && cleanProtocol != "" && cleanProtocol != "websocket" {
		return marshalRouteJSON(map[string]any{"ok": false, "error": "cf entry only supports websocket speed test"})
	}
	if isRouteCFEntry(item) {
		cleanProtocol = "websocket"
	}
	results := routeRelaySpeedTestDefault(endpoint, cleanProtocol, routeRelaySpeedTestBytes)
	rateBPS := int64(0)
	okResult := false
	for _, result := range results {
		if result.OK {
			okResult = true
			if result.RateBPS > rateBPS {
				rateBPS = result.RateBPS
			}
		}
	}
	status := "unreachable"
	if okResult {
		status = "tested"
	} else if len(results) == 0 {
		status = "no_result"
	}
	remoteSpeedDebug := fetchLinkRemoteSpeedDebugAfterTest(item, endpoint, cleanProtocol)
	return marshalRouteJSON(map[string]any{
		"ok":                 okResult,
		"route_id":           strings.TrimSpace(item.RouteID),
		"route_name":         strings.TrimSpace(item.Name),
		"status":             status,
		"protocol":           cleanProtocol,
		"rate_bps":           rateBPS,
		"source":             "active_speed_test",
		"results":            results,
		"remote_speed_debug": remoteSpeedDebug,
		"updated_at":         time.Now().UTC().Format(time.RFC3339),
	})
}

func loadRouteEndpointByID(configDir string, routeID string) (routeServerItem, routeEndpoint, error) {
	items, err := loadRoutes(configDir)
	if err != nil {
		return routeServerItem{}, routeEndpoint{}, err
	}
	item, ok := findRouteItemByID(routeID, items)
	if !ok {
		return routeServerItem{}, routeEndpoint{}, errors.New("route not found")
	}
	endpoint, err := resolveRouteEndpoint(item)
	if err != nil {
		return item, routeEndpoint{}, err
	}
	return item, endpoint, nil
}

func loadRoutes(configDir string) ([]routeServerItem, error) {
	configDir = strings.TrimSpace(configDir)
	if configDir == "" {
		return nil, errors.New("config dir is required")
	}
	return []routeServerItem{}, nil
}

func findRouteItemByID(routeID string, items []routeServerItem) (routeServerItem, bool) {
	cleanID := strings.TrimSpace(routeID)
	if cleanID == "" {
		return routeServerItem{}, false
	}
	for _, item := range items {
		if strings.EqualFold(strings.TrimSpace(item.RouteID), cleanID) || strings.EqualFold(strings.TrimSpace(item.ClientEntryID), cleanID) {
			return item, true
		}
	}
	return routeServerItem{}, false
}

func resolveRouteEndpoint(item routeServerItem) (routeEndpoint, error) {
	routeID := strings.TrimSpace(item.RouteID)
	if routeID == "" {
		return routeEndpoint{}, errors.New("route_id is required")
	}
	route := buildRoutePath(item)
	if len(route) == 0 {
		return routeEndpoint{}, fmt.Errorf("route path is empty: %s", routeID)
	}
	entryNodeID := strings.TrimSpace(route[0])
	entryHost := ""
	entryPort := 0
	routeLayer := normalizeRouteLayer(strings.TrimSpace(item.RouteLayer))
	for _, hop := range item.HopConfigs {
		hopNodeID := normalizeRouteNodeID(strconv.Itoa(hop.NodeNo))
		if hopNodeID == "" || hopNodeID != normalizeRouteNodeID(entryNodeID) {
			continue
		}
		entryHost = strings.TrimSpace(hop.RelayHost)
		if hop.ExternalPort > 0 {
			entryPort = hop.ExternalPort
		} else if hop.ListenPort > 0 {
			entryPort = hop.ListenPort
		}
		routeLayer = normalizeRouteLayer(firstNonEmptyString(strings.TrimSpace(hop.RouteLayer), strings.TrimSpace(item.RouteLayer)))
		break
	}
	if entryHost == "" {
		return routeEndpoint{}, fmt.Errorf("selected route entry host is unavailable: %s", routeID)
	}
	if entryPort <= 0 {
		return routeEndpoint{}, fmt.Errorf("selected route entry port is unavailable: %s", routeID)
	}
	return routeEndpoint{
		RouteID:             effectiveRouteRelayRouteID(item),
		RouteName:           strings.TrimSpace(item.Name),
		EntryNodeID:         entryNodeID,
		EntryHost:           entryHost,
		EntryPort:           entryPort,
		RouteLayer:          routeLayer,
		RouteSecret:         strings.TrimSpace(item.Secret),
		AuthTicket:          strings.TrimSpace(item.AuthTicket),
		PreserveRelayDomain: isRouteCFEntry(item),
	}, nil
}

func buildRoutePath(item routeServerItem) []string {
	route := make([]string, 0, 2+len(item.CascadeNodeIDs))
	seen := make(map[string]struct{}, 2+len(item.CascadeNodeIDs))
	push := func(raw string) {
		nodeID := normalizeRouteNodeID(raw)
		if nodeID == "" {
			return
		}
		if _, exists := seen[nodeID]; exists {
			return
		}
		seen[nodeID] = struct{}{}
		route = append(route, nodeID)
	}
	push(item.EntryNodeID)
	for _, id := range item.CascadeNodeIDs {
		push(id)
	}
	push(item.ExitNodeID)
	return route
}

func normalizeRouteNodeID(raw string) string {
	value := strings.TrimSpace(raw)
	if value == "" {
		return ""
	}
	lower := strings.ToLower(value)
	if strings.HasPrefix(lower, "node-") || strings.HasPrefix(lower, "node_") {
		suffix := strings.TrimPrefix(strings.TrimPrefix(lower, "node-"), "node_")
		suffix = strings.TrimSpace(suffix)
		if suffix != "" {
			if n, err := strconv.Atoi(suffix); err == nil && n > 0 {
				return strconv.Itoa(n)
			}
			return suffix
		}
	}
	if n, err := strconv.Atoi(value); err == nil && n > 0 {
		return strconv.Itoa(n)
	}
	return value
}

func effectiveRouteRelayRouteID(item routeServerItem) string {
	if relayID := strings.TrimSpace(item.RelayRouteID); relayID != "" {
		return relayID
	}
	return strings.TrimSpace(item.RouteID)
}

func normalizeRouteLayer(raw string) string {
	switch strings.ToLower(strings.TrimSpace(raw)) {
	case "", "auto", "default", "http", "http2", "h2", "http3", "h3":
		return ""
	case "websocket", "ws", "wss":
		return "websocket"
	case "websocket-h3", "ws-h3", "h3-websocket", "h3-ws":
		return "websocket-h3"
	default:
		return ""
	}
}

func routeRelayProtocolCandidates(layer string) []string {
	switch normalizeRouteLayer(layer) {
	case "websocket":
		return []string{"websocket"}
	case "websocket-h3":
		return []string{"websocket-h3"}
	default:
		return []string{"websocket-h3", "websocket"}
	}
}

func routeReachabilityProtocolsForEndpoint(item routeServerItem, endpoint routeEndpoint) []string {
	if isRouteCFEntry(item) {
		return []string{"websocket"}
	}
	candidates := routeRelayProtocolCandidates(endpoint.RouteLayer)
	out := make([]string, 0, len(candidates))
	seen := map[string]struct{}{}
	for _, candidate := range candidates {
		protocol := normalizeRouteLayer(candidate)
		if !isRouteRelaySupportedProtocol(protocol) {
			continue
		}
		if _, ok := seen[protocol]; ok {
			continue
		}
		seen[protocol] = struct{}{}
		out = append(out, protocol)
	}
	if len(out) == 0 {
		return []string{"websocket-h3", "websocket"}
	}
	return out
}

func isRouteRelaySupportedProtocol(protocol string) bool {
	switch normalizeRouteLayer(protocol) {
	case "websocket", "websocket-h3":
		return true
	default:
		return false
	}
}

func isRouteCFEntry(item routeServerItem) bool {
	for _, value := range []string{item.ClientEntryType, item.ClientEntryID, item.RouteID, item.Name} {
		clean := strings.ToLower(strings.TrimSpace(value))
		if clean == "cf" || strings.HasSuffix(clean, "_cf") {
			return true
		}
	}
	return false
}

func linkProtocolOrder(protocol string) int {
	if normalizeRouteLayer(protocol) == "websocket-h3" {
		return 0
	}
	return 1
}

func linkPingPongProbe(endpoint routeEndpoint, protocol string) (time.Duration, error) {
	const payloadBytes = 64
	conn, err := openRouteRelayConn(endpoint, protocol, routeRelayProtocolProbeTimeout)
	if err != nil {
		return 0, err
	}
	defer conn.Close()
	stream, err := openLinkPingPongStream(conn, payloadBytes)
	if err != nil {
		return 0, err
	}
	defer stream.Close()
	payload := make([]byte, payloadBytes)
	for i := range payload {
		payload[i] = byte((i * 31) % 251)
	}
	echo := make([]byte, payloadBytes)
	startedAt := time.Now()
	_ = stream.SetDeadline(time.Now().Add(routeRelayProtocolProbeTimeout))
	if _, err := stream.Write(payload); err != nil {
		_ = stream.SetDeadline(time.Time{})
		return 0, err
	}
	if _, err := io.ReadFull(stream, echo); err != nil {
		_ = stream.SetDeadline(time.Time{})
		return 0, err
	}
	_ = stream.SetDeadline(time.Time{})
	if !bytes.Equal(payload, echo) {
		return 0, errors.New("ping-pong echo mismatch")
	}
	elapsed := time.Since(startedAt)
	if elapsed <= 0 {
		return time.Millisecond, nil
	}
	return elapsed, nil
}

func openLinkPingPongStream(conn net.Conn, payloadBytes int64) (net.Conn, error) {
	session, err := newMobileRouteFrameClient(conn)
	if err != nil {
		return nil, err
	}
	req := routeTunnelOpenRequest{Type: routeRelayModePingPong, PingBytes: payloadBytes, Priority: "realtime"}
	stream, err := session.OpenWithRequest(req, linkPortForwardResponseReadTimeout)
	if err != nil {
		_ = session.Close()
		return nil, err
	}
	return &linkPingPongStreamConn{Conn: stream, session: session}, nil
}

type linkPingPongStreamConn struct {
	net.Conn
	session *mobileRouteFrameSession
}

func (c *linkPingPongStreamConn) Close() error {
	var firstErr error
	if c != nil && c.Conn != nil {
		firstErr = c.Conn.Close()
	}
	if c != nil && c.session != nil {
		if err := c.session.Close(); firstErr == nil {
			firstErr = err
		}
	}
	return firstErr
}

func openRouteRelayConn(endpoint routeEndpoint, protocol string, openTimeout time.Duration) (net.Conn, error) {
	return openRouteRelayConnWithMode(endpoint, protocol, openTimeout, routeRelayModeBridge)
}

func openRouteRelayDataStreamConn(endpoint routeEndpoint, protocol string, openTimeout time.Duration) (net.Conn, error) {
	return openRouteRelayConnWithMode(endpoint, protocol, openTimeout, routeRelayModeStream)
}

func openRouteRelayConnWithMode(endpoint routeEndpoint, protocol string, openTimeout time.Duration, mode string) (net.Conn, error) {
	switch normalizeRouteLayer(protocol) {
	case "websocket":
		return openRouteRelayWebSocketConn(endpoint, openTimeout, mode, nil)
	case "websocket-h3":
		return openRouteRelayHTTP3WebSocketConn(endpoint, openTimeout, mode, nil)
	default:
		return nil, fmt.Errorf("unsupported relay protocol: %s", protocol)
	}
}

func openRouteRelayWebSocketConn(endpoint routeEndpoint, openTimeout time.Duration, mode string, extraHeaders http.Header) (net.Conn, error) {
	relayDialHost, relayHostHeader, err := resolveLinkDialHostWithPolicy(endpoint.EntryHost, endpoint.PreserveRelayDomain)
	if err != nil {
		return nil, err
	}
	if openTimeout <= 0 {
		openTimeout = routeRelayProtocolProbeTimeout
	}
	relayURL, err := buildRouteRelayWebSocketURL(relayHostHeader, endpoint.EntryPort, endpoint.RouteID)
	if err != nil {
		return nil, err
	}
	header, err := buildRouteRelayHeaders(endpoint, firstNonEmptyString(strings.TrimSpace(mode), routeRelayModeBridge), linkBridgeRoleToNext, 0)
	if err != nil {
		return nil, err
	}
	for k, values := range extraHeaders {
		for _, value := range values {
			header.Add(k, value)
		}
	}
	dialHostPort := net.JoinHostPort(relayDialHost, strconv.Itoa(endpoint.EntryPort))
	dialer := websocket.Dialer{
		HandshakeTimeout:  openTimeout,
		Proxy:             nil,
		ReadBufferSize:    routeRelayWebSocketBufferBytes,
		WriteBufferSize:   routeRelayWebSocketBufferBytes,
		EnableCompression: false,
		NetDialContext: func(ctx context.Context, network string, addr string) (net.Conn, error) {
			netDialer := net.Dialer{Timeout: openTimeout}
			return netDialer.DialContext(ctx, network, dialHostPort)
		},
		TLSClientConfig: &tls.Config{
			MinVersion:         tls.VersionTLS12,
			ServerName:         resolveLinkTLSServerName(relayDialHost, relayHostHeader),
			InsecureSkipVerify: true,
		},
	}
	ws, response, err := dialer.Dial(relayURL, header)
	if err != nil {
		if response != nil && response.Body != nil {
			body, _ := io.ReadAll(io.LimitReader(response.Body, 1024))
			_ = response.Body.Close()
			return nil, fmt.Errorf("probe relay websocket failed: status=%d body=%s", response.StatusCode, strings.TrimSpace(string(body)))
		}
		return nil, wrapRouteDialError("websocket", relayDialHost, endpoint.EntryPort, err)
	}
	return newWebSocketNetConn(ws), nil
}

func openRouteRelayHTTP3WebSocketConn(endpoint routeEndpoint, openTimeout time.Duration, mode string, extraHeaders http.Header) (net.Conn, error) {
	relayDialHost, relayHostHeader, err := resolveLinkDialHostWithPolicy(endpoint.EntryHost, endpoint.PreserveRelayDomain)
	if err != nil {
		return nil, err
	}
	if openTimeout <= 0 {
		openTimeout = routeRelayProtocolProbeTimeout
	}
	relayURL, err := buildRouteRelayURL(relayHostHeader, endpoint.EntryPort, endpoint.RouteID)
	if err != nil {
		return nil, err
	}
	ctx, cancel := context.WithTimeout(context.Background(), openTimeout)
	dialHostPort := net.JoinHostPort(relayDialHost, strconv.Itoa(endpoint.EntryPort))
	tlsConf := &tls.Config{
		MinVersion:         tls.VersionTLS13,
		NextProtos:         []string{http3.NextProtoH3},
		ServerName:         resolveLinkTLSServerName(relayDialHost, relayHostHeader),
		InsecureSkipVerify: true,
	}
	quicConn, err := quic.DialAddr(ctx, dialHostPort, tlsConf, newLinkQUICConfig())
	if err != nil {
		cancel()
		return nil, wrapRouteDialError("websocket-h3", relayDialHost, endpoint.EntryPort, err)
	}
	transport := &http3.Transport{}
	clientConn := transport.NewClientConn(quicConn)
	select {
	case <-clientConn.ReceivedSettings():
	case <-ctx.Done():
		_ = quicConn.CloseWithError(0, "h3 websocket settings timeout")
		cancel()
		return nil, fmt.Errorf("probe relay h3 websocket open timeout: relay=%s:%d", relayDialHost, endpoint.EntryPort)
	case <-clientConn.Context().Done():
		cancel()
		return nil, fmt.Errorf("probe relay h3 websocket failed: %w", context.Cause(clientConn.Context()))
	}
	if settings := clientConn.Settings(); settings == nil || !settings.EnableExtendedConnect {
		_ = quicConn.CloseWithError(0, "h3 websocket extended connect disabled")
		cancel()
		return nil, errors.New("probe relay h3 websocket failed: server did not enable extended connect")
	}
	stream, err := clientConn.OpenRequestStream(ctx)
	if err != nil {
		_ = quicConn.CloseWithError(0, "h3 websocket stream open failed")
		cancel()
		return nil, wrapRouteDialError("websocket-h3", relayDialHost, endpoint.EntryPort, err)
	}
	request, err := http.NewRequestWithContext(ctx, http.MethodConnect, relayURL, nil)
	if err != nil {
		cancelRouteH3Stream(stream, quicConn, cancel, "h3 websocket request build failed")
		return nil, err
	}
	request.Proto = "websocket"
	request.ProtoMajor = 3
	request.ProtoMinor = 0
	request.Header, err = buildRouteRelayHeaders(endpoint, firstNonEmptyString(strings.TrimSpace(mode), routeRelayModeBridge), linkBridgeRoleToNext, 0)
	if err != nil {
		cancelRouteH3Stream(stream, quicConn, cancel, "h3 websocket auth failed")
		return nil, err
	}
	for k, values := range extraHeaders {
		for _, value := range values {
			request.Header.Add(k, value)
		}
	}
	if strings.TrimSpace(relayHostHeader) != "" {
		request.Host = strings.TrimSpace(relayHostHeader)
	}
	if err := stream.SendRequestHeader(request); err != nil {
		cancelRouteH3Stream(stream, quicConn, cancel, "h3 websocket header send failed")
		return nil, wrapRouteDialError("websocket-h3", relayDialHost, endpoint.EntryPort, err)
	}
	response, err := stream.ReadResponse()
	if err != nil {
		cancelRouteH3Stream(stream, quicConn, cancel, "h3 websocket response failed")
		return nil, wrapRouteDialError("websocket-h3", relayDialHost, endpoint.EntryPort, err)
	}
	if response.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(io.LimitReader(response.Body, 1024))
		_ = response.Body.Close()
		cancelRouteH3Stream(stream, quicConn, cancel, "h3 websocket status failed")
		return nil, fmt.Errorf("probe relay h3 websocket failed: status=%d body=%s", response.StatusCode, strings.TrimSpace(string(body)))
	}
	cancelOnce := sync.Once{}
	return &linkHTTP3StreamNetConn{
		stream: stream,
		local:  dummyAddr("probe-route-h3-local"),
		remote: dummyAddr(dialHostPort),
		closeFn: func() error {
			var closeErr error
			cancelOnce.Do(func() {
				cancel()
				stream.CancelRead(quic.StreamErrorCode(http3.ErrCodeRequestCanceled))
				stream.CancelWrite(quic.StreamErrorCode(http3.ErrCodeRequestCanceled))
				closeErr = quicConn.CloseWithError(0, "h3 websocket closed")
			})
			return closeErr
		},
	}, nil
}

func routeRelaySpeedTestDefault(endpoint routeEndpoint, protocol string, byteCount int64) []routeSpeedTestResult {
	candidates := routeRelaySpeedTestCandidates(endpoint.RouteLayer, protocol)
	if byteCount <= 0 {
		byteCount = routeRelaySpeedTestBytes
	}
	if byteCount > routeRelaySpeedTestMaxBytes {
		byteCount = routeRelaySpeedTestMaxBytes
	}
	results := make([]routeSpeedTestResult, 0, len(candidates))
	for _, candidate := range candidates {
		results = append(results, routeRelaySpeedTestWithLayer(endpoint, candidate, byteCount, routeRelaySpeedTestTimeout))
	}
	return results
}

func routeRelaySpeedTestCandidates(layer string, protocol string) []string {
	cleanProtocol := normalizeRouteLayer(protocol)
	switch cleanProtocol {
	case "websocket", "websocket-h3":
		return []string{cleanProtocol}
	}
	return routeRelayProtocolCandidates(layer)
}

func fetchLinkRemoteSpeedDebugAfterTest(item routeServerItem, endpoint routeEndpoint, protocol string) map[string]any {
	var lastPayload routeSpeedDebugResultPayload
	var lastErr error
	for idx, wait := range []time.Duration{0, 450 * time.Millisecond, 1200 * time.Millisecond} {
		if wait > 0 {
			time.Sleep(wait)
		}
		payload, err := routeRelayFetchSpeedDebugDefault(endpoint, protocol, 1200*time.Millisecond)
		if err != nil {
			lastErr = err
			continue
		}
		lastPayload = payload
		if routeSpeedDebugPayloadHasRoute(payload, item) || idx == 2 {
			return map[string]any{
				"ok":      true,
				"source":  "relay_entry",
				"remote":  payload,
				"fetched": time.Now().UTC().Format(time.RFC3339),
			}
		}
	}
	if strings.TrimSpace(lastPayload.Type) != "" || lastPayload.OK || len(lastPayload.Active) > 0 || len(lastPayload.Recent) > 0 {
		return map[string]any{
			"ok":      true,
			"source":  "relay_entry",
			"remote":  lastPayload,
			"fetched": time.Now().UTC().Format(time.RFC3339),
		}
	}
	errText := "remote speed debug fetch failed"
	if lastErr != nil {
		errText = lastErr.Error()
	}
	return map[string]any{
		"ok":      false,
		"source":  "relay_entry",
		"error":   errText,
		"fetched": time.Now().UTC().Format(time.RFC3339),
	}
}

func routeRelayFetchSpeedDebugDefault(endpoint routeEndpoint, protocol string, openTimeout time.Duration) (routeSpeedDebugResultPayload, error) {
	candidates := routeRelaySpeedTestCandidates(endpoint.RouteLayer, protocol)
	if len(candidates) == 0 {
		candidates = routeRelayProtocolCandidates(endpoint.RouteLayer)
	}
	var errs []string
	for _, candidate := range candidates {
		payload, err := routeRelayFetchSpeedDebugWithLayer(endpoint, candidate, openTimeout)
		if err == nil {
			return payload, nil
		}
		errs = append(errs, fmt.Sprintf("%s=%v", normalizeRouteLayer(candidate), err))
	}
	if len(errs) == 0 {
		return routeSpeedDebugResultPayload{}, errors.New("no relay speed debug protocol candidate")
	}
	return routeSpeedDebugResultPayload{}, fmt.Errorf("relay speed debug fetch failed: %s", strings.Join(errs, "; "))
}

func routeRelayFetchSpeedDebugWithLayer(endpoint routeEndpoint, layer string, openTimeout time.Duration) (routeSpeedDebugResultPayload, error) {
	cleanLayer := normalizeRouteLayer(layer)
	if cleanLayer != "websocket" && cleanLayer != "websocket-h3" {
		return routeSpeedDebugResultPayload{}, fmt.Errorf("unsupported speed debug protocol: %s", layer)
	}
	if openTimeout <= 0 {
		openTimeout = routeRelayProtocolProbeTimeout
	}
	conn, err := openRouteRelayConnWithMode(endpoint, cleanLayer, openTimeout, routeRelayModeSpeedDebug)
	if err != nil {
		return routeSpeedDebugResultPayload{}, err
	}
	defer conn.Close()
	_ = conn.SetReadDeadline(time.Now().Add(openTimeout))
	var payload routeSpeedDebugResultPayload
	if err := json.NewDecoder(conn).Decode(&payload); err != nil {
		return routeSpeedDebugResultPayload{}, err
	}
	_ = conn.SetReadDeadline(time.Time{})
	if strings.TrimSpace(payload.Scope) == "" {
		payload.Scope = "route_relay"
	}
	return payload, nil
}

func routeSpeedDebugPayloadHasRoute(payload routeSpeedDebugResultPayload, item routeServerItem) bool {
	match := make(map[string]struct{})
	for _, id := range routeIDMatchVariants(item.RouteID, item.RelayRouteID, item.ClientEntryID) {
		match[id] = struct{}{}
	}
	if len(match) == 0 {
		return false
	}
	samples := append(append([]routeSpeedDebugItemPayload{}, payload.Active...), payload.Recent...)
	for _, sample := range samples {
		for _, id := range routeIDMatchVariants(sample.RouteID) {
			if _, ok := match[id]; ok {
				return true
			}
		}
	}
	return false
}

func routeIDMatchVariants(values ...string) []string {
	seen := make(map[string]struct{})
	out := make([]string, 0, len(values)*3)
	add := func(value string) {
		clean := strings.TrimSpace(value)
		if clean == "" {
			return
		}
		lower := strings.ToLower(clean)
		variants := []string{clean, lower}
		if strings.HasSuffix(lower, "_pub") && len(clean) > 4 {
			variants = append(variants, clean[:len(clean)-4], lower[:len(lower)-4])
		}
		if strings.HasSuffix(lower, "-pub") && len(clean) > 4 {
			variants = append(variants, clean[:len(clean)-4], lower[:len(lower)-4])
		}
		for _, variant := range variants {
			key := strings.ToLower(strings.TrimSpace(variant))
			if key == "" {
				continue
			}
			if _, ok := seen[key]; ok {
				continue
			}
			seen[key] = struct{}{}
			out = append(out, key)
		}
	}
	for _, value := range values {
		add(value)
	}
	return out
}

func routeRelaySpeedTestWithLayer(endpoint routeEndpoint, layer string, byteCount int64, timeout time.Duration) routeSpeedTestResult {
	startedAt := time.Now()
	result := routeSpeedTestResult{
		Protocol:       normalizeRouteLayer(layer),
		StartedAt:      startedAt.UTC().Format(time.RFC3339),
		LocalStartedAt: startedAt.UTC().Format(time.RFC3339),
	}
	if byteCount <= 0 {
		byteCount = routeRelaySpeedTestBytes
	}
	if byteCount > routeRelaySpeedTestMaxBytes {
		byteCount = routeRelaySpeedTestMaxBytes
	}
	result.RequestedBytes = byteCount
	if timeout <= 0 {
		timeout = routeRelaySpeedTestTimeout
	}
	deadlineAt := startedAt.Add(timeout)
	speedConn, speedErr := openRouteRelaySpeedTestConn(endpoint, layer, byteCount, timeout)
	headerAt := time.Now()
	if speedErr != nil {
		result.LatencyMS = durationMilliseconds(headerAt.Sub(startedAt))
		result.OpenHandshakeMS = result.LatencyMS
		result.Error = speedErr.Error()
		result.EndedAt = time.Now().UTC().Format(time.RFC3339)
		return result
	}
	defer speedConn.Close()
	result.OpenHandshakeMS = durationMilliseconds(headerAt.Sub(startedAt))
	consumeRouteRelaySpeedTestData(speedConn, byteCount, time.Until(deadlineAt), &result)
	return result
}

func openRouteRelaySpeedTestConn(endpoint routeEndpoint, layer string, byteCount int64, openTimeout time.Duration) (net.Conn, error) {
	switch normalizeRouteLayer(layer) {
	case "websocket":
		return openRouteRelayWebSocketSpeedTestConn(endpoint, byteCount, openTimeout)
	case "websocket-h3":
		return openRouteRelayHTTP3WebSocketSpeedTestConn(endpoint, byteCount, openTimeout)
	default:
		return nil, fmt.Errorf("unsupported speed test protocol: %s", layer)
	}
}

func openRouteRelayWebSocketSpeedTestConn(endpoint routeEndpoint, byteCount int64, openTimeout time.Duration) (net.Conn, error) {
	relayDialHost, relayHostHeader, err := resolveLinkDialHostWithPolicy(endpoint.EntryHost, endpoint.PreserveRelayDomain)
	if err != nil {
		return nil, err
	}
	if openTimeout <= 0 {
		openTimeout = routeRelaySpeedTestTimeout
	}
	relayURL, err := buildRouteRelayWebSocketURL(relayHostHeader, endpoint.EntryPort, endpoint.RouteID)
	if err != nil {
		return nil, err
	}
	header, err := buildRouteRelayHeaders(endpoint, routeRelayModeSpeedTest, "", byteCount)
	if err != nil {
		return nil, err
	}
	dialHostPort := net.JoinHostPort(relayDialHost, strconv.Itoa(endpoint.EntryPort))
	dialer := websocket.Dialer{
		HandshakeTimeout:  openTimeout,
		Proxy:             nil,
		ReadBufferSize:    routeRelayWebSocketBufferBytes,
		WriteBufferSize:   routeRelayWebSocketBufferBytes,
		EnableCompression: false,
		NetDialContext: func(ctx context.Context, network string, addr string) (net.Conn, error) {
			netDialer := net.Dialer{Timeout: openTimeout}
			return netDialer.DialContext(ctx, network, dialHostPort)
		},
		TLSClientConfig: &tls.Config{
			MinVersion:         tls.VersionTLS12,
			ServerName:         resolveLinkTLSServerName(relayDialHost, relayHostHeader),
			InsecureSkipVerify: true,
		},
	}
	ws, response, err := dialer.Dial(relayURL, header)
	if err != nil {
		if response != nil && response.Body != nil {
			body, _ := io.ReadAll(io.LimitReader(response.Body, 1024))
			_ = response.Body.Close()
			return nil, fmt.Errorf("probe relay websocket speed test failed: status=%d body=%s", response.StatusCode, strings.TrimSpace(string(body)))
		}
		return nil, wrapRouteDialError("websocket", relayDialHost, endpoint.EntryPort, err)
	}
	return newWebSocketNetConn(ws), nil
}

func openRouteRelayHTTP3WebSocketSpeedTestConn(endpoint routeEndpoint, byteCount int64, openTimeout time.Duration) (net.Conn, error) {
	relayDialHost, relayHostHeader, err := resolveLinkDialHostWithPolicy(endpoint.EntryHost, endpoint.PreserveRelayDomain)
	if err != nil {
		return nil, err
	}
	if openTimeout <= 0 {
		openTimeout = routeRelaySpeedTestTimeout
	}
	relayURL, err := buildRouteRelayURL(relayHostHeader, endpoint.EntryPort, endpoint.RouteID)
	if err != nil {
		return nil, err
	}
	ctx, cancel := context.WithTimeout(context.Background(), openTimeout)
	dialHostPort := net.JoinHostPort(relayDialHost, strconv.Itoa(endpoint.EntryPort))
	tlsConf := &tls.Config{
		MinVersion:         tls.VersionTLS13,
		NextProtos:         []string{http3.NextProtoH3},
		ServerName:         resolveLinkTLSServerName(relayDialHost, relayHostHeader),
		InsecureSkipVerify: true,
	}
	quicConn, err := quic.DialAddr(ctx, dialHostPort, tlsConf, newLinkQUICConfig())
	if err != nil {
		cancel()
		return nil, wrapRouteDialError("websocket-h3", relayDialHost, endpoint.EntryPort, err)
	}
	transport := &http3.Transport{}
	clientConn := transport.NewClientConn(quicConn)
	select {
	case <-clientConn.ReceivedSettings():
	case <-ctx.Done():
		_ = quicConn.CloseWithError(0, "h3 speed websocket settings timeout")
		cancel()
		return nil, fmt.Errorf("probe relay h3 websocket open timeout: relay=%s:%d", relayDialHost, endpoint.EntryPort)
	case <-clientConn.Context().Done():
		cancel()
		return nil, fmt.Errorf("probe relay h3 websocket failed: %w", context.Cause(clientConn.Context()))
	}
	if settings := clientConn.Settings(); settings == nil || !settings.EnableExtendedConnect {
		_ = quicConn.CloseWithError(0, "h3 websocket extended connect disabled")
		cancel()
		return nil, errors.New("probe relay h3 websocket failed: server did not enable extended connect")
	}
	stream, err := clientConn.OpenRequestStream(ctx)
	if err != nil {
		_ = quicConn.CloseWithError(0, "h3 speed websocket stream open failed")
		cancel()
		return nil, wrapRouteDialError("websocket-h3", relayDialHost, endpoint.EntryPort, err)
	}
	request, err := http.NewRequestWithContext(ctx, http.MethodConnect, relayURL, nil)
	if err != nil {
		cancelRouteH3Stream(stream, quicConn, cancel, "h3 speed websocket request build failed")
		return nil, err
	}
	request.Proto = "websocket"
	request.ProtoMajor = 3
	request.ProtoMinor = 0
	request.Header, err = buildRouteRelayHeaders(endpoint, routeRelayModeSpeedTest, "", byteCount)
	if err != nil {
		cancelRouteH3Stream(stream, quicConn, cancel, "h3 speed websocket auth failed")
		return nil, err
	}
	if strings.TrimSpace(relayHostHeader) != "" {
		request.Host = strings.TrimSpace(relayHostHeader)
	}
	if err := stream.SendRequestHeader(request); err != nil {
		cancelRouteH3Stream(stream, quicConn, cancel, "h3 speed websocket header send failed")
		return nil, wrapRouteDialError("websocket-h3", relayDialHost, endpoint.EntryPort, err)
	}
	response, err := stream.ReadResponse()
	if err != nil {
		cancelRouteH3Stream(stream, quicConn, cancel, "h3 speed websocket response failed")
		return nil, wrapRouteDialError("websocket-h3", relayDialHost, endpoint.EntryPort, err)
	}
	if response.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(io.LimitReader(response.Body, 1024))
		_ = response.Body.Close()
		cancelRouteH3Stream(stream, quicConn, cancel, "h3 speed websocket status failed")
		return nil, fmt.Errorf("probe relay h3 websocket failed: status=%d body=%s", response.StatusCode, strings.TrimSpace(string(body)))
	}
	cancelOnce := sync.Once{}
	return &linkHTTP3StreamNetConn{
		stream: stream,
		local:  dummyAddr("probe-route-h3-speed-local"),
		remote: dummyAddr(dialHostPort),
		closeFn: func() error {
			var closeErr error
			cancelOnce.Do(func() {
				cancel()
				stream.CancelRead(quic.StreamErrorCode(http3.ErrCodeRequestCanceled))
				stream.CancelWrite(quic.StreamErrorCode(http3.ErrCodeRequestCanceled))
				closeErr = quicConn.CloseWithError(0, "h3 speed websocket closed")
			})
			return closeErr
		},
	}, nil
}

func consumeRouteRelaySpeedTestData(reader io.Reader, byteCount int64, maxDuration time.Duration, result *routeSpeedTestResult) {
	if result == nil {
		return
	}
	readStartedAt := time.Now()
	result.LocalStartedAt = readStartedAt.UTC().Format(time.RFC3339)
	result.ReadChunkBytes = int64(routeRelayWebSocketBufferBytes)
	if maxDuration <= 0 {
		maxDuration = routeRelaySpeedTestTimeout
	}
	if deadliner, ok := reader.(interface{ SetReadDeadline(time.Time) error }); ok {
		_ = deadliner.SetReadDeadline(readStartedAt.Add(maxDuration))
		defer deadliner.SetReadDeadline(time.Time{})
	}
	var first [1]byte
	firstN, firstErr := io.ReadFull(reader, first[:])
	firstAt := time.Now()
	if firstN > 0 {
		result.LatencyMS = durationMilliseconds(firstAt.Sub(readStartedAt))
		result.FirstByteMS = result.LatencyMS
		result.LocalFirstByteAt = firstAt.UTC().Format(time.RFC3339)
		result.ReadCalls++
		result.TotalReadBlockMS += result.LatencyMS
		result.LastReadBlockMS = result.LatencyMS
		result.MaxReadBlockMS = result.LatencyMS
	}
	remaining := byteCount - int64(firstN)
	if remaining < 0 {
		remaining = 0
	}
	n, err := copyRouteRelaySpeedTestData(io.LimitReader(reader, remaining), result)
	endedAt := time.Now()
	result.EndedAt = endedAt.UTC().Format(time.RFC3339)
	result.LocalCompletedAt = result.EndedAt
	result.Bytes = int64(firstN) + n
	result.DurationMS = durationMilliseconds(endedAt.Sub(readStartedAt))
	if result.ReadCalls > 0 {
		result.AvgReadBytes = result.Bytes / result.ReadCalls
	}
	if firstErr != nil {
		if isRouteSpeedTestDurationLimitErr(firstErr, result.Bytes) {
			finalizeRouteSpeedTestResult(result, readStartedAt, endedAt)
			return
		}
		result.Error = firstErr.Error()
		return
	}
	if err != nil {
		if isRouteSpeedTestDurationLimitErr(err, result.Bytes) {
			finalizeRouteSpeedTestResult(result, readStartedAt, endedAt)
			return
		}
		result.Error = err.Error()
		return
	}
	if result.Bytes <= 0 {
		result.Error = "speed test returned no data"
		return
	}
	if result.Bytes < byteCount {
		result.Error = fmt.Sprintf("speed test returned incomplete data: bytes=%d want=%d", result.Bytes, byteCount)
		return
	}
	finalizeRouteSpeedTestResult(result, readStartedAt, endedAt)
}

func finalizeRouteSpeedTestResult(result *routeSpeedTestResult, startedAt time.Time, endedAt time.Time) {
	if result.Bytes <= 0 {
		result.Error = "speed test returned no data"
		return
	}
	elapsed := endedAt.Sub(startedAt)
	if elapsed <= 0 {
		elapsed = time.Millisecond
	}
	result.RateBPS = int64(float64(result.Bytes) / elapsed.Seconds())
	result.OK = true
	result.Error = ""
}

func copyRouteRelaySpeedTestData(reader io.Reader, result *routeSpeedTestResult) (int64, error) {
	if reader == nil {
		return 0, nil
	}
	buf := make([]byte, routeRelayWebSocketBufferBytes)
	var total int64
	for {
		startedAt := time.Now()
		n, err := reader.Read(buf)
		elapsedMS := durationMilliseconds(time.Since(startedAt))
		if n > 0 {
			total += int64(n)
			if result != nil {
				result.ReadCalls++
				result.TotalReadBlockMS += elapsedMS
				result.LastReadBlockMS = elapsedMS
				if elapsedMS > result.MaxReadBlockMS {
					result.MaxReadBlockMS = elapsedMS
				}
			}
		}
		if err != nil {
			if errors.Is(err, io.EOF) {
				return total, nil
			}
			return total, err
		}
		if n == 0 {
			return total, io.ErrNoProgress
		}
	}
}

func isRouteSpeedTestDurationLimitErr(err error, bytesRead int64) bool {
	if err == nil || bytesRead <= 0 {
		return false
	}
	var netErr net.Error
	if errors.As(err, &netErr) && netErr.Timeout() {
		return true
	}
	text := strings.ToLower(strings.TrimSpace(err.Error()))
	return strings.Contains(text, "timeout") || strings.Contains(text, "deadline") || errors.Is(err, context.DeadlineExceeded) || errors.Is(err, context.Canceled)
}

func buildRouteRelayHeaders(endpoint routeEndpoint, mode string, role string, speedBytes int64) (http.Header, error) {
	routeID := strings.TrimSpace(endpoint.RouteID)
	header := http.Header{}
	header.Set(routeLegacyRouteIDHeader, routeID)
	header.Set(routeCodexRouteIDHeader, routeID)
	header.Set(linkCodexVersionHeader, linkAuthPacketVersion)
	header.Set(linkCodexRelayModeHeader, strings.TrimSpace(mode))
	header.Set("X-Codex-Auth-Timestamp", time.Now().UTC().Format(time.RFC3339Nano))
	if strings.TrimSpace(role) != "" {
		header.Set(linkCodexRelayRoleHeader, normalizeRouteBridgeRole(role))
	}
	if speedBytes > 0 {
		header.Set(linkCodexSpeedBytesHeader, strconv.FormatInt(speedBytes, 10))
	}
	if ticket := strings.TrimSpace(endpoint.AuthTicket); ticket != "" {
		header.Set(linkCodexAuthTicketHeader, ticket)
	}
	if err := applyRouteSecretAuthHeaders(header, routeID, endpoint.RouteSecret); err != nil {
		return nil, err
	}
	return header, nil
}

func applyRouteSecretAuthHeaders(headers http.Header, routeID string, secret string) error {
	cleanRouteID := strings.TrimSpace(routeID)
	cleanSecret := strings.TrimSpace(secret)
	if cleanRouteID == "" {
		return errors.New("route_id is required")
	}
	if cleanSecret == "" {
		return errors.New("route_secret is required")
	}
	nonce := randomHexToken(16)
	headers.Set("Authorization", "Bearer "+nonce)
	headers.Set(linkCodexAuthModeHeader, "secret_hmac")
	headers.Set("X-Codex-Auth-Timestamp", time.Now().UTC().Format(time.RFC3339Nano))
	headers.Set(linkCodexMACHeader, buildLinkHMAC(cleanSecret, cleanRouteID, nonce))
	return nil
}

func buildLinkHMAC(secret string, routeID string, nonce string) string {
	mac := hmac.New(sha256.New, []byte(strings.TrimSpace(secret)))
	_, _ = mac.Write([]byte(strings.TrimSpace(routeID)))
	_, _ = mac.Write([]byte("\n"))
	_, _ = mac.Write([]byte(strings.TrimSpace(nonce)))
	return hex.EncodeToString(mac.Sum(nil))
}

func buildRouteRelayURL(host string, port int, routeID string) (string, error) {
	return buildRouteRelayURLWithScheme("https", host, port, routeID)
}

func buildRouteRelayWebSocketURL(host string, port int, routeID string) (string, error) {
	return buildRouteRelayURLWithScheme("wss", host, port, routeID)
}

func buildRouteRelayURLWithScheme(scheme string, host string, port int, routeID string) (string, error) {
	cleanHost := strings.TrimSpace(strings.Trim(host, "[]"))
	if cleanHost == "" {
		return "", errors.New("empty relay host")
	}
	if port <= 0 || port > 65535 {
		return "", errors.New("invalid relay port")
	}
	u := &url.URL{
		Scheme: scheme,
		Host:   net.JoinHostPort(cleanHost, strconv.Itoa(port)),
		Path:   routeRelayAPIPath,
	}
	query := u.Query()
	query.Set("route_id", strings.TrimSpace(routeID))
	u.RawQuery = query.Encode()
	return u.String(), nil
}

func resolveLinkDialHost(rawHost string) (dialHost string, hostHeader string, err error) {
	return resolveLinkDialHostWithPolicy(rawHost, false)
}

func resolveLinkDialHostWithPolicy(rawHost string, preserveDomain bool) (dialHost string, hostHeader string, err error) {
	cleanHost := strings.TrimSpace(strings.Trim(rawHost, "[]"))
	if cleanHost == "" {
		return "", "", errors.New("empty relay host")
	}
	if parsed := net.ParseIP(cleanHost); parsed != nil {
		ipText := parsed.String()
		return ipText, ipText, nil
	}
	if preserveDomain {
		return cleanHost, cleanHost, nil
	}
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	ips, err := net.DefaultResolver.LookupIP(ctx, "ip", cleanHost)
	if err != nil {
		return "", "", fmt.Errorf("resolve relay host failed: %w", err)
	}
	ip := selectLinkPreferredDialIP(ips)
	if ip == nil {
		return "", "", errors.New("resolve relay host failed: no ip")
	}
	ipText := ip.String()
	return ipText, ipText, nil
}

func resolveLinkTLSServerName(dialHost string, hostHeader string) string {
	for _, candidate := range []string{dialHost, hostHeader} {
		clean := strings.TrimSpace(strings.Trim(candidate, "[]"))
		if clean == "" {
			continue
		}
		return clean
	}
	return ""
}

func selectLinkPreferredDialIP(ips []net.IP) net.IP {
	for _, candidate := range ips {
		if candidate == nil {
			continue
		}
		if v4 := candidate.To4(); v4 != nil {
			return v4
		}
	}
	for _, candidate := range ips {
		if candidate == nil {
			continue
		}
		if v6 := candidate.To16(); v6 != nil {
			return v6
		}
	}
	return nil
}

func wrapRouteDialError(protocol string, host string, port int, err error) error {
	if err == nil {
		return nil
	}
	return fmt.Errorf("probe relay %s dial failed: relay=%s:%d: %w", normalizeRouteLayer(protocol), strings.TrimSpace(host), port, err)
}

func newLinkQUICConfig() *quic.Config {
	return &quic.Config{
		Versions:                       []quic.Version{quic.Version2, quic.Version1},
		EnableDatagrams:                true,
		KeepAlivePeriod:                10 * time.Second,
		InitialStreamReceiveWindow:     routeRelayQUICInitialStreamWindow,
		MaxStreamReceiveWindow:         routeRelayQUICMaxStreamWindow,
		InitialConnectionReceiveWindow: routeRelayQUICInitialConnWindow,
		MaxConnectionReceiveWindow:     routeRelayQUICMaxConnWindow,
	}
}

func normalizeRouteBridgeRole(raw string) string {
	if strings.EqualFold(strings.TrimSpace(raw), "to_prev") {
		return "to_prev"
	}
	return linkBridgeRoleToNext
}

func durationMilliseconds(elapsed time.Duration) int64 {
	if elapsed <= 0 {
		return 1
	}
	ms := int64(elapsed / time.Millisecond)
	if ms <= 0 {
		return 1
	}
	return ms
}

func firstNonEmptyString(values ...string) string {
	for _, value := range values {
		if strings.TrimSpace(value) != "" {
			return value
		}
	}
	return ""
}

func marshalRouteJSON(value any) string {
	raw, err := json.Marshal(value)
	if err != nil {
		return `{"ok":false,"error":"json marshal failed"}`
	}
	return string(raw)
}

func cancelRouteH3Stream(stream *http3.RequestStream, conn *quic.Conn, cancel context.CancelFunc, reason string) {
	stream.CancelRead(quic.StreamErrorCode(http3.ErrCodeRequestCanceled))
	stream.CancelWrite(quic.StreamErrorCode(http3.ErrCodeRequestCanceled))
	_ = conn.CloseWithError(0, reason)
	cancel()
}

type linkHTTP3StreamNetConn struct {
	stream  *http3.RequestStream
	local   net.Addr
	remote  net.Addr
	closeFn func() error
}

func (c *linkHTTP3StreamNetConn) Read(p []byte) (int, error) {
	return c.stream.Read(p)
}

func (c *linkHTTP3StreamNetConn) Write(p []byte) (int, error) {
	return c.stream.Write(p)
}

func (c *linkHTTP3StreamNetConn) Close() error {
	if c.closeFn != nil {
		return c.closeFn()
	}
	return nil
}

func (c *linkHTTP3StreamNetConn) LocalAddr() net.Addr {
	return c.local
}

func (c *linkHTTP3StreamNetConn) RemoteAddr() net.Addr {
	return c.remote
}

func (c *linkHTTP3StreamNetConn) SetDeadline(t time.Time) error {
	if err := c.stream.SetReadDeadline(t); err != nil {
		return err
	}
	return c.stream.SetWriteDeadline(t)
}

func (c *linkHTTP3StreamNetConn) SetReadDeadline(t time.Time) error {
	return c.stream.SetReadDeadline(t)
}

func (c *linkHTTP3StreamNetConn) SetWriteDeadline(t time.Time) error {
	return c.stream.SetWriteDeadline(t)
}
