...
- 前端页面构建产物需内嵌到 Go 可执行文件，发布形态为单可执行文件。
...
- In Scope
  ...
  - 前端静态资源构建后内嵌到 Go 二进制并由服务端托管。
  - 前端页面在现有技术架构下重构实现，禁止简单引用或批量移植历史前端代码。
- Out of Scope
...
- RQ-010 前端页面内嵌入 Go 可执行文件并以单可执行文件发布
...
- AC-007 单可执行文件启动后可直接访问内嵌前端页面并完成 SPA 路由回退。
...
### 选型六 前端交付形态
- 结论: 前端构建产物通过 `embed` 内嵌到 `manager_service` 二进制。
- 取舍: 放弃额外前端分发目录，换取单文件发布与部署简化。
- 约束: 发布前必须完成前端构建并通过内嵌资源可用性校验。
...
### 生命周期
- 启动阶段: 加载配置与凭据、初始化路由与服务、加载内嵌前端资源并绑定本机监听。
...
### 前端内嵌与发布接口
- `GET /` 与静态资源由内嵌前端产物提供。
- 非 API 路由回退至 `index.html` 以支持 SPA 导航。
...
### 编码功能约束 前端重构
- FC-FE-01 保留既有功能需求与业务语义，禁止以删减功能换取重构速度。
- FC-FE-02 前端保持 `React` + `Vite` + `TypeScript` + `fetch` 技术架构，不引入桌面 IPC 回流。
- FC-FE-03 仅允许通过 `manager_service` API 访问后端能力，禁止默认可执行的主控直连路径。
- FC-FE-04 采用模块化重构，不允许整文件复制旧实现后做局部改名。
- FC-FE-05 组件层不得直接承载跨域编排逻辑，统一下沉到 `services` 与 `hooks`。
- FC-FE-06 临时兼容分支必须显式标记 `deprecated` 与清退版本，不得作为长期方案。
- FC-FE-07 单可执行文件发布场景下必须保证前端静态资源与 API 版本同步可用。
- FC-FE-08 任一重构改动必须标注需求编号映射并回写跟踪表。
...
- PKG-W3-02 登录与会话管理
- PKG-W3-03 业务页接入与联调
- PKG-W3-04 前端构建产物内嵌与单可执行文件发布
- PKG-W3-05 前端重构治理 禁止移植式改造 保留功能语义
...
| RQ-010 | PKG-W3-04 W3-05 | TST-W3-02 单文件发布可用性验证 与前端重构约束审计 |
...
### G2-TECH 技术门
- T-01 端口 鉴权 迁移一致性 冻结约束与单文件发布风险均覆盖
- T-02 技术选型定版为 `gin`
