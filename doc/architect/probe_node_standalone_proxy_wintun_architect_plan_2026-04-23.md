...
- 日期: 2026-04-23
- 备注: 本文用于将 manager 现有驱动安装与代理接入能力收敛到 probe_node，后续 manager 计划下线，且 Linux 侧需完整支持 TUN 代理接管。
- 风险:
  - 本地管理监听端口从既有 16031 调整到 16032，若部署侧配置未同步可能导致可用性问题。
...
- 检查表:
  ...
  - [x] 已完成接口与门禁口径定义
- 跟踪表状态: 已完成（可持续演进）
- 结论记录: 采用 probe_node 新增本地管理监听 `127.0.0.1:16032` 默认可配置方案，内嵌登录页，驱动安装与代理接入由 probe_node 独立承载；Windows 与 Linux 均需提供完整 TUN 代理接管能力。
...
- 方案B: 使用 16032
- 结论: 选择方案B
- 依据: 用户明确要求 probe_node 改为新增 16032 监听，避免与现有 manager 16031 冲突。
