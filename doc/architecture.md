...
- **探针面板中心服务 (Probe Panel)**: 采用 **Go** 生态构建
...
graph TD
    ...
    subgraph "Probe Panel Service (Go)"
...
2. **直连 vs 代理**:
   - 对于普通的 API 请求与资产查询，请求发送至云端 Probe Panel 中心。
   - 对于 SSH 终端：可根据配置选择客户端 Go 进程**直接连接**远程主机，或者通过 Probe Panel 作为**跳板机/堡垒机代理**访问（适合闭环网络）。
3. **性能与监控**:
