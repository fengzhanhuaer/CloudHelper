...
export async function downloadNetworkRuleRoutes(baseURL: string, token: string): Promise<{ fileName: string; content: string }> {
  const payload = await callAdminWSRpc<NetworkRuleRoutesDownloadResponse>(
    baseURL,
    token,
    networkRuleRoutesDownloadAction,
    { file_name: "rule_routes.txt" },
    { timeoutMs: 30000 },
  );
  const fileName = String(payload.filename || payload.file_name || payload.path || payload.rule_file_path || "rule_routes.txt").trim() || "rule_routes.txt";
  const contentBase64 = String(payload.content_base64 || payload.file_content_base64 || "").trim();
