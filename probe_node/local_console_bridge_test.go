...
func TestProbeLocalConsoleTrustedBypass(t *testing.T) {
	...
	// With the trusted context (set only by the in-process controller bridge), it passes.
...
func TestRunProbeLocalConsoleBridgeServesPanel(t *testing.T) {
	t.Setenv("PROBE_NODE_DATA_DIR", t.TempDir())
	...
	go runProbeLocalConsoleBridge(msg, nodeIdentity{NodeID: "1"}, serverConn, encoder, &mu)

	_ = clientConn.SetReadDeadline(time.Now().Add(5 * time.Second))
	var res probeLocalConsoleBridgeResult
