...
type probeLocalConsoleBridgeResult struct {
...
var (
	probeLocalConsoleBridgeMuxOnce sync.Once
	probeLocalConsoleBridgeMux     http.Handler
)

// probeLocalConsoleBridgeHandler returns a cached local-console mux used to serve
...
// runProbeLocalConsoleBridge serves a single HTTP request (forwarded by the
// controller over the control channel) against the local console mux and returns
// the response back to the controller.
func runProbeLocalConsoleBridge(msg probeControlMessage, identity nodeIdentity, stream net.Conn, encoder *json.Encoder, writeMu *sync.Mutex) {
	requestID := strings.TrimSpace(msg.RequestID)
	if requestID == "" {
		return
	}
	result := probeLocalConsoleBridgeResult{
	...
	if path == "" || !strings.HasPrefix(path, "/") {
		...
		sendProbeLocalConsoleBridgeResult(stream, encoder, writeMu, result)
		return
	}
	...
	func() {
		defer func() {
			if rec := recover(); rec != nil {
				log.Printf("probe local console bridge panic: request_id=%s path=%s err=%v", requestID, path, rec)
				recorder.Code = http.StatusInternalServerError
			}
		}()
		probeLocalConsoleBridgeHandler().ServeHTTP(recorder, req)
	}()

	body := recorder.Body.Bytes()
	if len(body) > probeLocalConsoleBridgeMaxBodyBytes {
		body = body[:probeLocalConsoleBridgeMaxBodyBytes]
	}
...
// must not be forwarded across the bridge boundary.
...
func sendProbeLocalConsoleBridgeResult(stream net.Conn, encoder *json.Encoder, writeMu *sync.Mutex, result probeLocalConsoleBridgeResult) {
	if writeErr := writeProbeStreamJSON(stream, encoder, writeMu, result); writeErr != nil {
		log.Printf("probe local console bridge result send failed: request_id=%s err=%v", strings.TrimSpace(result.RequestID), writeErr)
	}
}
