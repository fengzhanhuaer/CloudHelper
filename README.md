...
主控 `/mng/probe` 的探针列表使用与普通探针相同的入口创建 `mihomo_exit` 节点：填写名称、选择“Mihomo 出口探针”并创建。创建只登记节点，不弹出安装方式；随后点击该节点行内独立的“安装”按钮，选择 Linux x64 原生命令或完整 Docker Compose。`/mng/route` 的“二次分流”Tab只配置已创建特殊探针的 Clash/Mihomo 订阅和二次分流规则。每个特殊探针自动聚合为一条普通虚拟路由规则；普通探针只看到域名/CIDR条目和特殊出口节点，不会收到订阅URL、请求头、代理节点或凭据。
