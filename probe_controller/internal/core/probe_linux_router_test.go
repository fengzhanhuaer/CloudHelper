...
func TestLinuxRouterInstallerIsAlpineOpenRCAndDualArchitecture(t *testing.T) {
	...
	for _, marker := range []string{"command -v apk", "apk add --no-cache", "gzip", "jq", "kmod", "rc-service rc-update ip nft sysctl", "command -v \"${required_command}\"", "modprobe tun", "/dev/net/tun", "ip tuntap add", "x86_64", "aarch64|arm64", "cloudhelper-probe-router-linux-${GOARCH}", "${PROGRAM_ASSET}-manifest.json", "PROBE_ROUTER_PROGRAM_URL", "PROBE_ROUTER_MANIFEST_URL", "/api/probe/proxy/download", "--data-urlencode \"node_id=${PROBE_NODE_ID}\"", "--data-urlencode \"secret=${PROBE_NODE_SECRET}\"", "X-CloudHelper-Download-URL: ${remote_url}", "/opt/cloudhelper/probe_router", "--upgrade-verify-build-kind=linux_router", "Mihomo archive sha256 mismatch", "${INSTALL_DIR}/data/mihomo", "rc-update add", "PROBE_LOCAL_LISTEN", "PROBE_LOCAL_CONSOLE_ENABLED", "0.0.0.0:16032", "curl -fsS --noproxy", "/local/router"} {
		if !strings.Contains(probeRouterInstallScriptLinux, marker) {
			t.Fatalf("router installer missing %q", marker)
		}
	}
	for _, forbidden := range []string{"systemctl", "docker", "PROBE_ROUTER_WEB_LISTEN", "0.0.0.0:18080"} {
